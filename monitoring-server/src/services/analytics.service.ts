...
export class AnalyticsService {
  ...
  async getTeamOverview(startDateStr?: string, endDateStr?: string, employeeName?: string): Promise<any> {
    const key = `${startDateStr || ''}|${endDateStr || ''}|${employeeName || ''}`;
    const now = Date.now();
    const hit = overviewCache.get(key);
    if (hit && now - hit.at < OVERVIEW_CACHE_TTL_MS) {
      return hit.data;
    }
    const data = await this.computeTeamOverview(startDateStr, endDateStr, employeeName);
    overviewCache.set(key, { at: now, data });
    // Opportunistic eviction so the map can't grow unbounded.
    if (overviewCache.size > 64) {
      for (const [k, v] of overviewCache) {
        if (now - v.at >= OVERVIEW_CACHE_TTL_MS) overviewCache.delete(k);
      }
    }
    return data;
  }
   ...
   */
  private async computeTeamOverview(startDateStr?: string, endDateStr?: string, employeeName?: string): Promise<any> {
