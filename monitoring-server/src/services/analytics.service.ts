...
// Short-lived in-memory cache for the (expensive) team overview. The dashboard
// hits /overview and /insights for the same range back-to-back, and refreshes
// every ~60s; caching for a few seconds means the heavy ActivityLog scan runs
// once per range instead of 2-3x.
...
export class AnalyticsService {
  ...
  async getAttendance(name: string, startDateStr: string, endDateStr: string): Promise<AttendanceSummary | null> {
    ...
    // Only the timing/duration fields are used below — project them so the
    // unbounded applications[]/browserTabs[] arrays aren't dragged over the wire.
   ...
   */
  async getProductivityTrend(days: number, name?: string, endDateStr?: string, allowedNames?: string[]): Promise<Array<{
    date: string;
    ...
    // Fetch the WHOLE range in ONE query and bucket by business-day in memory,
    // instead of running one query per day (was `days` sequential round-trips,
    // e.g. 30 for a monthly trend). Projected to only the fields we actually use
    // so unrelated columns aren't dragged along. The employeeId filter (when
    // scoped) lets Mongo use the {employeeId:1, timestamp:-1} compound index.
    const rangeStart = tzStartOfDay(addDaysStr(endStr, -(days - 1)));
    const rangeEnd = tzEndOfDay(endStr);
    const query: any = { timestamp: { $gte: rangeStart, $lte: rangeEnd } };
    if (employeeFilter) query.employeeId = employeeFilter._id;
    else if (allowedNames) {
      const allowed = await Employee.find({ name: { $in: allowedNames } }).select('_id').lean();
      query.employeeId = { $in: allowed.map(e => e._id) };
    }

    const logs = await ActivityLog.find(query)
      .select('timestamp workSeconds idleSeconds applications browserTabs')
      .lean();
    ...
    for (const l of logs) {
      const dayStr = toDateStr(new Date(l.timestamp as any));
      let b = byDay.get(dayStr);
      if (!b) {
        b = { work: 0, idle: 0, apps: [], tabs: [] };
        byDay.set(dayStr, b);
      }
      b.work += l.workSeconds;
      b.idle += l.idleSeconds;
      for (const a of l.applications) b.apps.push({ name: a.name, duration: a.duration || 0 });
      for (const t of l.browserTabs) b.tabs.push({ title: t.title, url: t.url, duration: t.duration || 0 });
    }

    const result = [];
    for (let i = days - 1; i >= 0; i--) {
      const dayStr = addDaysStr(endStr, -i);
      const b = byDay.get(dayStr) || { work: 0, idle: 0, apps: [], tabs: [] };
      const { score, categorySeconds } = computeProductivity(b.apps, b.tabs);
  ...
  /**
   * Team overview — cached front for `computeTeamOverview`. A 15s TTL means the
   * dashboard's back-to-back /overview + /insights calls (and the 60s refresh)
   * only pay for the heavy ActivityLog scan once per range.
   */
  async getTeamOverview(startDateStr?: string, endDateStr?: string, employeeName?: string, allowedNames?: string[]): Promise<any> {
    const key = `${startDateStr || ''}|${endDateStr || ''}|${employeeName || ''}|${(allowedNames || []).sort().join(',')}`;
    const now = Date.now();
    const hit = overviewCache.get(key);
    if (hit && now - hit.at < OVERVIEW_CACHE_TTL_MS) {
      return hit.data;
    }

    // /overview and /insights, retries, and several open admin tabs can all
    // ask for the same range at once. Share a single database scan instead of
    // launching identical large ActivityLog reads concurrently.
    const pending = overviewInFlight.get(key);
    if (pending) return pending;

    const computation = this.computeTeamOverview(startDateStr, endDateStr, employeeName, allowedNames)
      .then((data) => {
        overviewCache.set(key, { at: Date.now(), data });
        if (overviewCache.size > 64) {
          const expiry = Date.now() - OVERVIEW_CACHE_TTL_MS;
          for (const [cacheKey, value] of overviewCache) {
            if (value.at < expiry) overviewCache.delete(cacheKey);
          }
        }
        return data;
      })
      .finally(() => overviewInFlight.delete(key));
    overviewInFlight.set(key, computation);
    return computation;
  }
   ...
   */
  private async computeTeamOverview(startDateStr?: string, endDateStr?: string, employeeName?: string, allowedNames?: string[]): Promise<any> {
    ...
    const visibleEmployees = employees.filter((employee) =>
      ...
      (!allowedNames || allowedNames.some(name => name.toLocaleLowerCase() === employee.name.toLocaleLowerCase()))
    );
    const logs = await ActivityLog.find({
   ...
   */
  async getInsights(dateStr?: string, endDateStr?: string, employeeName?: string, allowedNames?: string[]): Promise<string[]> {
    const overview = await this.getTeamOverview(dateStr, endDateStr, employeeName, allowedNames);
