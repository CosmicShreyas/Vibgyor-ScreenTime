...
export class AnalyticsService {
   ...
   */
  async getTeamOverview(startDateStr?: string, endDateStr?: string): Promise<any> {
   ...
   */
  async getInsights(dateStr?: string, endDateStr?: string): Promise<string[]> {
    const overview = await this.getTeamOverview(dateStr, endDateStr);
    const insights: string[] = [];

    if (overview.present_count === 0) {
      insights.push('No employee activity was recorded for this day.');
      return insights;
    }
