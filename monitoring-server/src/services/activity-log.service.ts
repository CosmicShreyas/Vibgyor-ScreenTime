...
import { startOfDay as tzStartOfDay, endOfDay as tzEndOfDay, toDateStr } from '../utils/timezone';
...
export class ActivityLogService {
  ...
  async storeActivityLog(employeeId: string, payload: MonitoringPayload): Promise<ActivityLogModel> {
    ...
    try {
      ...
      const a = payload.activity as any;
      const tamper = payload.tamper;

      // Create activity log document (intensity/genuineness fields optional).
      const activityLog = new ActivityLog({
        ...
        mouseClicks: a.mouse_clicks,
        mouseDistancePx: a.mouse_distance_px,
        scrollEvents: a.scroll_events,
        keystrokesPerMin: a.keystrokes_per_min,
        mouseActivityPerMin: a.mouse_activity_per_min,
        suspectedFakeSeconds: a.suspected_fake_seconds,
        genuinenessScore: a.genuineness_score,
        suspicionReasons: Array.isArray(a.suspicion_reasons) && a.suspicion_reasons.length > 0 ? a.suspicion_reasons : undefined,
        idleReason: (payload as any).idle_reason,
        tamper: tamper && {
          processRestarted: !!tamper.process_restarted,
          clockJumpDetected: !!tamper.clock_jump_detected,
          pausedSeconds: tamper.paused_seconds || undefined,
          relaunchedByWatchdog: !!tamper.relaunched_by_watchdog,
          watchdogRestarted: !!tamper.watchdog_restarted,
          serverUnreachableSeconds: tamper.server_unreachable_seconds || undefined,
          autostartRestored: !!tamper.autostart_restored,
        },
      });
  ...
  async getActivityLogsByEmployee(employeeId: string, limit: number = 100): Promise<ActivityLogModel[]> {
    ...
    try {
      ...
      return logs.map(log => ({
        ...
        browser_tabs: log.browserTabs.map(tab => ({ browser: tab.browser || 'Unknown', title: tab.title, url: tab.url })),
  ...
  async getActivitySummaryForDate(
    ...
    try {
      // Day boundaries in the business timezone (IST) for the given instant.
      const dayStr = toDateStr(date);
      const startOfDay = tzStartOfDay(dayStr);
      const endOfDay = tzEndOfDay(dayStr);
