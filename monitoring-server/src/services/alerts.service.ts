...
export interface Alert {
  ...
  threshold: number;
  created_at: string;
  /** @deprecated Kept for older dashboard clients. Prefer idle_responses. */
  idle_reasons?: string[];
...
export class AlertsService {
  ...
  async evaluate(dateStr?: string, endDateStr?: string): Promise<Alert[]> {
    ...
    for (const employee of employees) {
      ...
      const idleResponses: IdleResponse[] = [];
      ...
      for (const l of logs) {
        ...
        if (idleReason) {
          const submittedAt = new Date(l.timestamp || l.intervalEnd).toISOString();
          idleResponses.push({
            reason: idleReason,
            submitted_at: submittedAt,
            interval_start: new Date(l.intervalStart || l.timestamp).toISOString(),
            interval_end: new Date(l.intervalEnd || l.timestamp).toISOString(),
          });
        }
      ...
      idleResponses.sort((a, b) => Date.parse(a.submitted_at) - Date.parse(b.submitted_at));
      const legacyIdleReasons = idleResponses.slice(-3).map((response) => response.reason);
      ...
      if (idle / 60 >= cfg.highIdleMinutes) {
        alerts.push(this.mk(employee.name, 'high_idle', 'warning', stampDay,
          ...
          Math.round(idle / 60), cfg.highIdleMinutes, legacyIdleReasons, activityTime, idleResponses));
      } else if (idleResponses.length > 0) {
  ...
  private mk(
    ...
    return {
      ...
      ...(idleResponses?.length ? { idle_responses: idleResponses } : {}),
    };
  }
