...
export class EODReportsService {
  ...
  async initialize(): Promise<void> {
    const reportTime = config.eodReportTime;
  ...
  async addConfig(clientId: string, employeeName: string, email: string): Promise<EODReportEntry> {
    try {
      ...
      // Enforce a single EOD report configuration per user/client. Previously the
      // uniqueness check keyed on {clientId, email}, which let the same employee be
      // added repeatedly (e.g. with an email variation) and receive duplicate reports.
      const existing = await EODReportConfig.findOne({ clientId });
      if (existing) {
        throw new Error(
          `An EOD report is already configured for this employee (${existing.email}). Edit or delete the existing one instead of adding another.`
        );
      }
   ...
   */
  private async generateEmployeeDayData(
    clientId: string,
    employeeName: string,
    targetDateStr?: string
  ): Promise<EmployeeDayData | null> {
    // Report on the given calendar day (business timezone). Defaults to the
    // previous completed day — the scheduled job runs at/after midnight, so
    // "the day just ended" is the correct subject, not the near-empty new day.
    const dateStr = targetDateStr || addDaysStr(toDateStr(new Date()), -1);
    const dayStart = tzStartOfDay(dateStr);
    const dayEnd = tzEndOfDay(dateStr);

    logger.info(`📊 Generating EOD report for ${employeeName} (${clientId}) for ${dateStr} (${dayStart.toISOString()} to ${dayEnd.toISOString()})`);
    ...
    if (logs.length === 0) {
      logger.info(`ℹ️ No activity data found for ${employeeName} on ${dateStr}`);
      return null;
    }
  ...
  async sendEODReport(
    ...
    try {
      ...
      const data = await this.generateEmployeeDayData(clientId, employeeName, targetDateStr);

      if (!data) {
        logger.warn(`⚠️ No data available for ${employeeName} on the requested day, skipping email`);
        return;
      }
