...
import { normalizeEmployeeId } from '../utils/employee-identity';
...
export type TimelineSegment = { start: Date; end: Date; type: 'work' | 'idle' | 'offline' };

/**
 * Build a NON-OVERLAPPING work/idle/offline timeline from activity logs for a
 * single day. Shared by both the all-employees and weekly timeline builders.
 *
 * Overlap bug this fixes: a log's `workSeconds + idleSeconds` can exceed the
 * real wall-clock span of its interval (client over-reporting after
 * resume/restart, anti-cheat accounting, or overlapping intervals). The old
 * code laid segments as `intervalStart + workSeconds`, `+ idleSeconds` with no
 * clamp, so idle segments ran far past their interval and later segments (and
 * offline gaps) painted on top of them — producing the "idle AND offline at the
 * same time" hatched-over-amber artifact.
 *
 * Fix: each interval's work+idle is proportionally clamped to fit within
 * `[intervalStart, intervalEnd]`, and `lastEndTime` only ever moves forward, so
 * segments are strictly sequential and never overlap.
 */
export function buildTimelineSegments(
  logs: Array<{ intervalStart: Date | string; intervalEnd: Date | string; workSeconds: number; idleSeconds: number }>,
  startOfDay: Date,
  endOfDay: Date,
  isToday: boolean
): TimelineSegment[] {
  ...
  if (logs.length === 0) {
    segments.push({ start: startOfDay, end: finalTime, type: 'offline' });
    return segments;
  }
  ...
  for (const log of logs) {
    ...
    // A segment can never start before where the previous one ended.
    const segStart = new Date(Math.max(intervalStart.getTime(), lastEndTime.getTime()));

    // Gap before this interval → offline.
    if (intervalStart.getTime() > lastEndTime.getTime()) {
      segments.push({ start: lastEndTime, end: intervalStart, type: 'offline' });
    }

    // The real wall-clock room available in this interval (never negative).
    const spanMs = Math.max(0, intervalEnd.getTime() - segStart.getTime());
    let workMs = Math.max(0, log.workSeconds) * 1000;
    let idleMs = Math.max(0, log.idleSeconds) * 1000;

    // Clamp reported work+idle to the interval span. If they overflow, scale
    // both down proportionally so they exactly fill the interval (and never spill
    // into the next one). This keeps the visual honest without dropping data.
    const totalMs = workMs + idleMs;
    if (totalMs > spanMs && totalMs > 0) {
      const scale = spanMs / totalMs;
      workMs = Math.round(workMs * scale);
      idleMs = Math.max(0, spanMs - workMs);
    }

    let cursor = segStart;
    if (workMs > 0) {
      const workEnd = new Date(cursor.getTime() + workMs);
      segments.push({ start: cursor, end: workEnd, type: 'work' });
      cursor = workEnd;
    }
    if (idleMs > 0) {
      const idleEnd = new Date(cursor.getTime() + idleMs);
      segments.push({ start: cursor, end: idleEnd, type: 'idle' });
      cursor = idleEnd;
    }

    // Advance the watermark to the furthest point reached (interval end at least,
    // so a zero-activity interval still consumes its span rather than showing as
    // an offline gap right after).
    lastEndTime = new Date(Math.max(cursor.getTime(), intervalEnd.getTime()));
  }

  // Trailing offline from the last activity to end-of-day (or now, if today).
  if (lastEndTime < finalTime) {
    segments.push({ start: lastEndTime, end: finalTime, type: 'offline' });
  }
...
export class EmployeeService {
  /**
   * Get or create an employee. When an employee ID is supplied it is the stable
   * identity; the name remains display data and may change over time.
   */
  async upsertEmployee(
    ...
    try {
      const updateData: any = {
        $set: { lastSeen: new Date(), monitoringPaused: false },
        $setOnInsert: { firstSeen: new Date() },
      };

      if (employeeIdKey) {
        updateData.$set.externalEmployeeId = employeeIdKey;
      }
      ...
      // Prefer the stable employee ID. For a legacy employee that has not yet
      // received an ID, adopt its existing name-based record so all historical
      // activity and screenshots remain attached to the same MongoDB _id.
      let existing = employeeIdKey
        ? await Employee.findOne({ externalEmployeeId: employeeIdKey })
        : null;
      if (!existing) {
        const legacyByName = await Employee.findOne({ name: trimmedName });
        if (legacyByName?.externalEmployeeId && employeeIdKey && legacyByName.externalEmployeeId !== employeeIdKey) {
          throw new Error(`Employee name '${trimmedName}' is already assigned to a different employee ID`);
        }
        existing = legacyByName;
      }

      if (existing && existing.name !== trimmedName) {
        const nameOwner = await Employee.findOne({ name: trimmedName });
        if (nameOwner && nameOwner._id.toString() !== existing._id.toString()) {
          throw new Error(`Employee name '${trimmedName}' is already assigned to another employee`);
        }
        updateData.$set.name = trimmedName;
      }

      const identityFilter = existing
        ? { _id: existing._id }
        : employeeIdKey
          ? { externalEmployeeId: employeeIdKey }
          : { name: trimmedName };

      if (!existing) updateData.$setOnInsert.name = trimmedName;
      ...
      try {
        ...
        // Two first submissions for the same employee can race. The unique
        // employee-ID index chooses the winner; retry against that record.
        if (employeeIdKey && error?.code === 11000) {
          employee = await Employee.findOneAndUpdate(
            { externalEmployeeId: employeeIdKey },
            { $set: updateData.$set },
            { new: true }
      ...
      if (!employee) {
        throw new Error(`Failed to resolve employee identity for '${trimmedName}'`);
      }
  ...
  async getEmployeeByEmployeeId(employeeId: string): Promise<any | null> {
    ...
    try {
      ...
      const employee = await Employee.findOne({ externalEmployeeId: employeeIdKey });
      if (employee) {
        return {
          id: employee._id.toString(),
          name: employee.name,
      ...
      const connectedClient = await ConnectedClient.findOne({ employeeIdKey });
   ...
   */
  async getAllEmployeesWithSummary(allowedEmployeeIds?: string[]): Promise<EmployeeSummary[]> {
    logger.info('Fetching all employees with summary data');

    try {
      const employeeQuery: any = allowedEmployeeIds ? { _id: { $in: allowedEmployeeIds } } : {};
      const employees = await Employee.find(employeeQuery).sort({ name: 1 }).lean();
      ...
      // One grouped query replaces the previous N+1 pattern (one query per
      // employee). With tens of employees that pattern saturated the Mongo
      // connection pool and made even the initial employee list take minutes.
      const todayTotals = await ActivityLog.aggregate([
        { $match: { timestamp: { $gte: startOfDay }, ...(allowedEmployeeIds ? { employeeId: { $in: allowedEmployeeIds } } : {}) } },
        {
          $group: {
            _id: '$employeeId',
      ...
      const totalsByEmployee = new Map(
        todayTotals.map((row) => [row._id.toString(), row])
      );

      const summaries = employees.map((employee) => {
          const totals = totalsByEmployee.get(employee._id.toString());
          const workTimeToday = totals?.workSeconds ?? 0;
          const idleTimeToday = totals?.idleSeconds ?? 0;
          ...
          let status: 'active' | 'idle' | 'offline' | 'paused';
          if (employee.monitoringPaused && minutesSinceUpdate < ACTIVE_HEARTBEAT_GRACE_MINUTES) {
            status = 'paused';
          } else if (minutesSinceUpdate < ACTIVE_HEARTBEAT_GRACE_MINUTES) {
            status = 'active';
          ...
          const summary = {
            ...
            status,
            monitoring_paused: !!employee.monitoringPaused,
            paused_at: employee.pausedAt,
          };
    ...
    try {
      ...
      todayLogs.forEach((log) => {
        log.browserTabs.forEach((tab: any) => {
          const tabKey = `${tab.browser || 'Unknown'}|${tab.title}|${tab.url}`;
   ...
   */
  async touchLastSeenByName(name: string, paused = false): Promise<void> {
    if (!name || !isValidEmployeeName(name)) return;
    try {
      const now = new Date();
      // Preserve the transition time rather than replacing pausedAt on every
      // heartbeat while the client remains paused.
      if (paused) {
        await Employee.updateOne(
          { name: name.trim(), monitoringPaused: { $ne: true } },
          { $set: { pausedAt: now } }
        );
      }
      await Employee.updateOne(
  ...
  async getApplicationUsage(name: string, period: string = 'today', startDateStr?: string, endDateStr?: string): Promise<any | null> {
    ...
    try {
      ...
      const logs = await ActivityLog.find(logQuery).select('applications').lean();
  ...
  async getBrowserTabUsage(name: string, period: string = 'today', startDateStr?: string, endDateStr?: string): Promise<any | null> {
    ...
    try {
      ...
      const logQuery: any = { timestamp: { $gte: startDate, $lte: endDate } };
      if (!isAll) logQuery.employeeId = employee!._id;
      const logs = await ActivityLog.find(logQuery).select('browserTabs').lean();

      // Aggregate durations by tab (title + url)
      const tabUsageMap = new Map<string, { browser: string; title: string; url: string; duration: number }>();
      let totalDuration = 0;
      ...
      logs.forEach((log) => {
        log.browserTabs.forEach((tab: any) => {
          ...
          if (duration > 0) {
            const browser = tab.browser || 'Unknown';
            const tabKey = `${browser}|${tab.title}|${tab.url}`;
            const existing = tabUsageMap.get(tabKey);
   ...
   */
  async getEmployeesTimeline(dateStr?: string, allowedEmployeeIds?: string[]): Promise<any[]> {
    ...
    try {
      ...
      // Fetch the day's logs once, then group in memory. This avoids one Mongo
      // round trip per employee while preserving the exact timeline output.
      const allLogs = await ActivityLog.find({
        timestamp: { $gte: startOfDay, $lte: endOfDay }, ...(allowedEmployeeIds ? { employeeId: { $in: allowedEmployeeIds } } : {}),
      })
        .sort({ employeeId: 1, intervalStart: 1 })
        .select('employeeId intervalStart intervalEnd workSeconds idleSeconds')
        .lean();
      ...
      const timelines = employees.map((employee) => {
          const logs = logsByEmployee.get(employee._id.toString()) ?? [];
          ...
          // Build non-overlapping work/idle/offline segments (see helper).
          const segments = buildTimelineSegments(
            logs,
            startOfDay,
            endOfDay,
            resolvedDateStr === todayStr
          );
  ...
  async getEmployeesTimelineRange(startDateStr: string, endDateStr: string): Promise<any[]> {
    const employees = (await Employee.find().sort({ name: 1 }).lean()).filter((employee) => {
      const isMongoId = /^[a-f0-9]{24}$/i.test(employee.name);
      const isUUID = /^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(employee.name);
    ...
    const allLogs = await ActivityLog.find({
      ...
      .sort({ employeeId: 1, intervalStart: 1 })
      .select('employeeId timestamp intervalStart intervalEnd workSeconds idleSeconds')
      .lean();
    const logsByEmployeeDay = new Map<string, typeof allLogs>();
    for (const log of allLogs) {
      const key = `${log.employeeId.toString()}|${toDateStr(new Date(log.timestamp))}`;
      const bucket = logsByEmployeeDay.get(key);
      if (bucket) bucket.push(log);
      else logsByEmployeeDay.set(key, [log]);
    }

    const timelines: any[] = [];
    const todayStr = toDateStr(new Date());
    for (let cursor = startDateStr, guard = 0; guard < 400; guard++) {
      const startOfDay = tzStartOfDay(cursor);
      const endOfDay = tzEndOfDay(cursor);
      for (const employee of employees) {
        const logs = logsByEmployeeDay.get(`${employee._id.toString()}|${cursor}`) ?? [];
        const minutesSinceUpdate = (Date.now() - employee.lastSeen.getTime()) / 60000;
        const status = employee.monitoringPaused && minutesSinceUpdate < ACTIVE_HEARTBEAT_GRACE_MINUTES
          ? 'paused'
          : minutesSinceUpdate < ACTIVE_HEARTBEAT_GRACE_MINUTES
          ? 'active'
          : minutesSinceUpdate < 60 ? 'idle' : 'offline';
        timelines.push({
          name: employee.name,
          date: cursor,
          status,
          work_time_today: logs.reduce((sum, log) => sum + log.workSeconds, 0),
          idle_time_today: logs.reduce((sum, log) => sum + log.idleSeconds, 0),
          segments: buildTimelineSegments(logs, startOfDay, endOfDay, cursor === todayStr),
        });
      }
  ...
  async getEmployeeWeeklyTimeline(name: string): Promise<any> {
    ...
    try {
      ...
      const rangeStart = tzStartOfDay(sixDaysAgoStr);
      const rangeEnd = tzEndOfDay(todayStr);
      const weeklyLogs = await ActivityLog.find({
        employeeId: employee._id,
        timestamp: { $gte: rangeStart, $lte: rangeEnd },
      })
        .sort({ intervalStart: 1 })
        .select('timestamp intervalStart intervalEnd workSeconds idleSeconds')
        .lean();
      const logsByDay = new Map<string, typeof weeklyLogs>();
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const logs = logsByDay.get(targetDateStr) ?? [];
  ...
  /**
   * Build an employee timesheet for an inclusive business-timezone date range.
   * Every calendar date is represented in `daily_breakdown`, including Sundays
   * (the configured week-off rule used throughout the reporting services).
   */
  async getTimesheetReport(
    startDateStr: string,
    endDateStr: string,
    allowedEmployeeIds?: string[],
    includeEmployeesWithoutActivity = true
  ): Promise<any[]> {
    logger.info(`Fetching timesheet report for ${startDateStr} to ${endDateStr}`);

    try {
      const dateKeys: string[] = [];
      for (let cursor = startDateStr; cursor <= endDateStr; cursor = addDaysStr(cursor, 1)) {
        dateKeys.push(cursor);
        if (dateKeys.length > 31) {
          throw new Error('Timesheet ranges cannot exceed 31 calendar days');
        }
      }

      const startDate = tzStartOfDay(startDateStr);
      const endDate = tzEndOfDay(endDateStr);
      const todayStr = toDateStr(new Date());
      const shiftSecondsPerDay = Math.max(0, (config.shiftEndHour - config.shiftStartHour) * 3600);
      const employees = await Employee.find(allowedEmployeeIds ? { _id: { $in: allowedEmployeeIds } } : {}).lean();
      const allLogs = await ActivityLog.find({
        employeeId: { $in: employees.map((employee) => employee._id) },
      ...
      for (const employee of employees) {
        const logs = logsByEmployee.get(employee._id.toString()) ?? [];
        if (!includeEmployeesWithoutActivity && logs.length === 0) continue;
        ...
        for (const log of logs) {
          ...
          const intervalStart = new Date(log.intervalStart);
          const intervalEnd = new Date(log.intervalEnd);
          if (!bucket.firstActivity || intervalStart < bucket.firstActivity) bucket.firstActivity = intervalStart;
          if (!bucket.lastActivity || intervalEnd > bucket.lastActivity) bucket.lastActivity = intervalEnd;
        ...
        const dailyBreakdown = dateKeys.map((dayKey) => {
          const bucket = perDay.get(dayKey) || {
            work: 0,
            idle: 0,
            firstActivity: null,
          ...
          const hasActivity = accounted > 0;
          const isWeekOff = weekdayOf(dayKey) === 0;
          const isUpcoming = dayKey > todayStr;
          const offline = !isWeekOff && hasActivity
            ? Math.max(0, shiftSecondsPerDay - accounted)
            : 0;

          totalWorkSeconds += bucket.work;
          totalIdleSeconds += bucket.idle;
          totalOfflineSeconds += offline;
          if (hasActivity) daysPresent += 1;
          if (isWeekOff) nonWorkingDays += 1;
          else if (!hasActivity && !isUpcoming) absentDays += 1;

          return {
            date: dayKey,
            is_working_day: !isWeekOff,
            status: isWeekOff
              ? (hasActivity ? 'week_off_worked' : 'week_off')
              : (hasActivity ? 'present' : (isUpcoming ? 'upcoming' : 'absent')),
            first_activity: bucket.firstActivity,
            last_activity: bucket.lastActivity,
            productive_hours: bucket.work / 3600,
            idle_hours: bucket.idle / 3600,
            offline_hours: offline / 3600,
            total_hours: (bucket.work + bucket.idle + offline) / 3600,
          };
        });

        timesheetData.push({
          employee_name: employee.name,
          first_activity: logs.length > 0 ? logs[0].intervalStart : null,
          last_activity: logs.length > 0 ? logs[logs.length - 1].intervalEnd : null,
          days_present: daysPresent,
          working_days: dateKeys.length - nonWorkingDays,
      ...
      const filteredTimesheetData = timesheetData.filter((data) => {
        ...
        if (isClientId) logger.debug(`Filtering out client ID from timesheet: ${data.employee_name}`);
        return !isClientId;
      });

      logger.info(`Generated timesheet report for ${filteredTimesheetData.length} employees`);
      return filteredTimesheetData;
    } catch (error) {
      logger.error('Failed to generate timesheet report:', error);
      throw error;
    }
  }

  /** Backwards-compatible monthly endpoint used by older dashboards. */
  async getMonthlyTimesheetReport(
    ...
    return this.getTimesheetReport(monthStartStr, monthEndStr, allowedEmployeeIds, false);
  }
