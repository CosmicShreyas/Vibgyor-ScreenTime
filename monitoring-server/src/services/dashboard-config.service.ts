...
interface DashboardConfig {
  ...
  adminEmails: string[];
  /** Recipient for employee-information update OTP requests from clients. */
  otpAdminEmail: string;
}
...
export class DashboardConfigService {
  ...
  async loadConfig(): Promise<DashboardConfig> {
    ...
    try {
      ...
      // Migrate existing installations once: preserve the old environment
      // value, then manage it from dashboard-config.json from this point on.
      if (typeof this.config!.otpAdminEmail !== 'string') {
        this.config!.otpAdminEmail = (process.env.ADMIN_EMAIL || '').trim();
      ...
      this.config = {
        ...
        adminEmails: [],
        otpAdminEmail: (process.env.ADMIN_EMAIL || '').trim(),
      };
      await this.saveConfig(this.config);
      return this.config;
    }
  ...
  async getCurrentConfig(): Promise<Omit<DashboardConfig, 'password'>> {
    ...
    return {
      ...
      otpAdminEmail: config.otpAdminEmail,
    };
  }
  ...
  async updateConfig(updates: {
    ...
    if (updates.otpAdminEmail !== undefined) {
      config.otpAdminEmail = updates.otpAdminEmail.trim();
      logger.info('Employee update OTP admin email updated');
    }
