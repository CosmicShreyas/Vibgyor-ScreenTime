...
export class ScreenshotService {
  ...
  async getScreenshotsWithFilters(
  ...
  ): Promise<{ items: Array<ScreenshotModel & { employee_name: string }>; total: number }> {
    logger.debug(`Fetching screenshots with filters: ${startDate} to ${endDate}, employee: ${employeeName || 'all'}, limit ${limit}, offset ${offset}`);
    ...
    try {
      ...
      // Total count for pagination, plus just the requested page. The client
      // gallery renders one page at a time — no reason to ship the whole range.
      ...
      const items = screenshots.map((screenshot: any) => ({
        id: screenshot._id.toString(),
  ...
  /**
   * One compact aggregate per employee for the evidence-vault landing view.
   * Mongo computes the count and newest capture, so the browser never has to
   * download every screenshot record simply to render employee cards.
   */
  async getScreenshotSummaries(
    startDate: Date,
    endDate: Date,
    allowedEmployeeIds?: string[]
  ): Promise<Array<{ employee_name: string; latest_screenshot: { id: string; captured_at: Date }; total_count: number }>> {
    try {
      const rows = await Screenshot.aggregate([
        { $match: { capturedAt: { $gte: startDate, $lte: endDate }, ...(allowedEmployeeIds ? { employeeId: { $in: allowedEmployeeIds.map(id => new mongoose.Types.ObjectId(id)) } } : {}) } },
        { $sort: { capturedAt: -1 } },
        {
          $group: {
            _id: '$employeeId',
            total_count: { $sum: 1 },
            latest_id: { $first: '$_id' },
            latest_captured_at: { $first: '$capturedAt' },
          },
        },
        {
          $lookup: {
      ...
      logger.error('Failed to fetch screenshot summaries:', error);
      throw error;
    }
  }

  /**
   * Lightweight screenshot INDEX for the timeline tooltip's "nearest capture"
   * thumbnail lookup. Returns ONLY {id, employee_name, captured_at} — no
   * file_path/file_size/created_at/expires_at — so the payload is a fraction of
   * the full list. Optionally scoped to one employee (the timeline hover only
   * needs the currently-selected employee's captures, not all 40k).
   */
  async getScreenshotIndex(
    startDate: Date,
    endDate: Date,
    employeeName?: string,
    limit: number = 4000,
    allowedEmployeeIds?: string[]
  ): Promise<Array<{ id: string; employee_name: string; captured_at: Date }>> {
    try {
      const query: any = { capturedAt: { $gte: startDate, $lte: endDate }, ...(allowedEmployeeIds ? { employeeId: { $in: allowedEmployeeIds } } : {}) };
      let empName = employeeName;
      if (employeeName) {
        const { Employee } = await import('../database/schemas');
        const employee = await Employee.findOne({ name: employeeName }).lean();
        if (!employee) return [];
        if (allowedEmployeeIds && !allowedEmployeeIds.includes(employee._id.toString())) return [];
        query.employeeId = employee._id;
        empName = (employee as any).name;
      }

      const rows = await Screenshot.find(query)
        .sort({ capturedAt: -1 })
        .limit(Math.max(1, Math.min(limit, 10000)))
        .select('capturedAt employeeId')
        .populate('employeeId', 'name')
        .lean();

      return rows.map((r: any) => ({
        id: r._id.toString(),
        employee_name: empName ?? r.employeeId?.name ?? '',
        captured_at: r.capturedAt,
      }));
    } catch (error) {
      logger.error('Failed to fetch screenshot index:', error);
      throw error;
    }
  }
