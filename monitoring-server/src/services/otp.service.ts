...
export interface EmployeeOTPVerificationResult {
  success: boolean;
  message: string;
  /** Present only after successful verification; consumed by the server route. */
  employeeName?: string;
  employeeId?: string;
}
...
class EmployeeOTPService {
  ...
  async requestOTP(clientId: string, employeeName: string, employeeId: string): Promise<{ success: boolean; message: string }> {
    try {
      const dashboardConfig = await dashboardConfigService.getCurrentConfig();
      // dashboard-config.json is authoritative. Older environment values are
      // copied there by DashboardConfigService during its one-time migration.
      const adminEmail = dashboardConfig.otpAdminEmail;

      if (!adminEmail) {
        logger.error('Employee update OTP admin email is not configured');
        return {
          success: false,
   ...
   * Verify OTP
   */
  verifyOTP(clientId: string, otp: string): EmployeeOTPVerificationResult {
