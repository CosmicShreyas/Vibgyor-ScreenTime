import mongoose, { Schema, Document } from 'mongoose';

/**
 * Employee Document Interface
 */
export interface IEmployee extends Document {
  _id: mongoose.Types.ObjectId;
  name: string;
  /** Stable employee identifier entered during client setup (canonical form). */
  externalEmployeeId?: string;
  location?: {
    city: string;
    state: string;
    country: string;
  };
  firstSeen: Date;
  lastSeen: Date;
  monitoringPaused: boolean;
  pausedAt?: Date;
  /** Managers this employee reports to. Multiple values support matrix teams. */
  managerIds: mongoose.Types.ObjectId[];
  /** @deprecated Legacy single-manager field retained for zero-downtime migration. */
  managerId?: mongoose.Types.ObjectId;
  createdAt: Date;
  updatedAt: Date;
}

/**
 * Activity Log Document Interface
 */
export interface IActivityLog extends Document {
  _id: mongoose.Types.ObjectId;
  employeeId: mongoose.Types.ObjectId;
  timestamp: Date;
  intervalStart: Date;
  intervalEnd: Date;
  workSeconds: number;
  idleSeconds: number;
  applications: Array<{
    name: string;
    duration: number;
  }>;
  browserTabs: Array<{
    browser?: string;
    title: string;
    url?: string;
    duration: number;
  }>;
  // Intensity + genuineness (anti-cheat). All optional/privacy-safe.
  keystrokes?: number;
  mouseClicks?: number;
  mouseDistancePx?: number;
  scrollEvents?: number;
  keystrokesPerMin?: number;
  mouseActivityPerMin?: number;
  suspectedFakeSeconds?: number;
  genuinenessScore?: number; // 0-100
  suspicionReasons?: string[];
  idleReason?: string;
  tamper?: {
    processRestarted?: boolean;
    clockJumpDetected?: boolean;
    pausedSeconds?: number;
    relaunchedByWatchdog?: boolean;
    watchdogRestarted?: boolean;
    serverUnreachableSeconds?: number;
    autostartRestored?: boolean;
  };
  createdAt: Date;
}

/**
 * Screenshot Document Interface
 */
export interface IScreenshot extends Document {
  _id: mongoose.Types.ObjectId;
  employeeId: mongoose.Types.ObjectId;
  activityLogId: mongoose.Types.ObjectId;
  filePath: string;
  fileSize: number;
  capturedAt: Date;
  expiresAt: Date;
  createdAt: Date;
}

/**
 * Connected Client Document Interface
 */
export interface IConnectedClient extends Document {
  _id: mongoose.Types.ObjectId;
  clientId: string;
  employeeName?: string;
  employeeId?: string;
  /** Canonical employee ID used to reconnect replacement installations. */
  employeeIdKey?: string;
  status?: 'online' | 'offline';
  lastOfflineAlertSentAt?: Date;
  systemInfo?: {
    osName: string;
    osVersion: string;
    hostname: string;
    cpuModel: string;
    cpuCores: number;
    totalRamGb: number;
    totalDiskGb: number;
    architecture: string;
  };
  resourceUsage?: {
    sampledAt: Date;
    cpuPercent: number;
    memoryUsedBytes: number;
    memoryTotalBytes: number;
    diskUsedBytes: number;
    diskTotalBytes: number;
  };
  firstSeen: Date;
  lastSeen: Date;
  createdAt: Date;
  updatedAt: Date;
}

/**
 * Client Config Document Interface
 */
export interface IClientConfig extends Document {
  _id: mongoose.Types.ObjectId;
  employeeName: string;
  serverUrl: string;
  authToken: string;
  screenshotIntervalMinutes: number;
  dataSendIntervalMinutes: number;
  locationUpdateIntervalMinutes: number;
  idleThresholdSeconds: number;
  appUsagePollIntervalSeconds: number;
  screenshotQuality: number;
  logLevel: string;
  fileDownloadPath: string;
  version: number;
  createdAt: Date;
  updatedAt: Date;
}

/** A dashboard account belonging to an employee who manages a team. */
export interface IManagerAccount extends Document {
  _id: mongoose.Types.ObjectId;
  employeeId: mongoose.Types.ObjectId;
  email: string;
  passwordHash: string;
  createdAt: Date;
  updatedAt: Date;
}

export interface IServerConfig extends Document {
  key: string;
  port: number;
  nodeEnv: string;
  mongodbUri: string;
  screenshotStoragePath: string;
  screenshotTtlDays: number;
  clientAuthToken: string;
  jwtSecret: string;
  logLevel: string;
  shiftStartHour: number;
  shiftEndHour: number;
  fileUploadPath: string;
  maxFileUploadSizeMb: number;
  eodReportTime: string;
  createdAt: Date;
  updatedAt: Date;
}

export interface IServerStatistic extends Document {
  timestamp: Date;
  cpuPercent: number;
  memoryUsedBytes: number;
  memoryTotalBytes: number;
  diskUsedBytes: number;
  diskTotalBytes: number;
  expiresAt: Date;
}

/**
 * Employee Schema
 */
const employeeSchema = new Schema<IEmployee>({
  name: {
    type: String,
    required: true,
    unique: true,
    index: true,
  },
  location: {
    city: { type: String, required: false },
    state: { type: String, required: false },
    country: { type: String, required: false },
  },
  firstSeen: {
    type: Date,
    required: true,
    default: Date.now,
  },
  lastSeen: {
    type: Date,
    required: true,
    default: Date.now,
    index: true,
  },
  externalEmployeeId: {
    type: String,
    required: false,
  },
  monitoringPaused: {
    type: Boolean,
    required: true,
    default: false,
    index: true,
  },
  pausedAt: {
    type: Date,
    required: false,
  },
  managerId: {
    type: Schema.Types.ObjectId,
    ref: 'Employee',
    required: false,
    index: true,
  },
  managerIds: {
    type: [{ type: Schema.Types.ObjectId, ref: 'Employee' }],
    required: true,
    default: [],
    index: true,
  },
}, {
  timestamps: true,
  collection: 'employees',
});

/**
 * Activity Log Schema
 */
const activityLogSchema = new Schema<IActivityLog>({
  employeeId: {
    type: Schema.Types.ObjectId,
    ref: 'Employee',
    required: true,
    index: true,
  },
  timestamp: {
    type: Date,
    required: true,
    index: true,
  },
  intervalStart: {
    type: Date,
    required: true,
    index: true,
  },
  intervalEnd: {
    type: Date,
    required: true,
  },
  workSeconds: {
    type: Number,
    required: true,
    min: 0,
  },
  idleSeconds: {
    type: Number,
    required: true,
    min: 0,
  },
  applications: [{
    name: { type: String, required: true },
    duration: { type: Number, required: true },
  }],
  browserTabs: [{
    browser: { type: String, required: false, default: 'Unknown' },
    title: { type: String, required: true },
    url: { type: String, required: false, default: '' },
    duration: { type: Number, required: true },
  }],
  // Intensity + genuineness (anti-cheat) — optional, backward-compatible.
  keystrokes: { type: Number, required: false },
  mouseClicks: { type: Number, required: false },
  mouseDistancePx: { type: Number, required: false },
  scrollEvents: { type: Number, required: false },
  keystrokesPerMin: { type: Number, required: false },
  mouseActivityPerMin: { type: Number, required: false },
  suspectedFakeSeconds: { type: Number, required: false },
  genuinenessScore: { type: Number, required: false, min: 0, max: 100 },
  suspicionReasons: { type: [String], required: false, default: undefined },
  idleReason: { type: String, required: false },
  tamper: {
    processRestarted: { type: Boolean, required: false },
    clockJumpDetected: { type: Boolean, required: false },
    pausedSeconds: { type: Number, required: false, min: 0 },
    relaunchedByWatchdog: { type: Boolean, required: false },
    watchdogRestarted: { type: Boolean, required: false },
    serverUnreachableSeconds: { type: Number, required: false, min: 0 },
    autostartRestored: { type: Boolean, required: false },
  },
}, {
  timestamps: { createdAt: true, updatedAt: false },
  collection: 'activity_logs',
});

// Compound index for common queries
activityLogSchema.index({ employeeId: 1, timestamp: -1 });
// Global dashboard/analytics date-range queries need the inverse ordering.
activityLogSchema.index({ timestamp: -1, employeeId: 1 });

/**
 * Screenshot Schema
 */
const screenshotSchema = new Schema<IScreenshot>({
  employeeId: {
    type: Schema.Types.ObjectId,
    ref: 'Employee',
    required: true,
    index: true,
  },
  activityLogId: {
    type: Schema.Types.ObjectId,
    ref: 'ActivityLog',
    required: true,
    index: true,
  },
  filePath: {
    type: String,
    required: true,
  },
  fileSize: {
    type: Number,
    required: true,
    min: 1,
  },
  capturedAt: {
    type: Date,
    required: true,
    index: true,
  },
  expiresAt: {
    type: Date,
    required: true,
    index: true,
  },
}, {
  timestamps: { createdAt: true, updatedAt: false },
  collection: 'screenshots',
});

// Compound index for common queries
screenshotSchema.index({ employeeId: 1, capturedAt: -1 });
// Supports global evidence-vault range scans and newest-first ordering.
screenshotSchema.index({ capturedAt: -1, employeeId: 1 });

/**
 * Connected Client Schema
 */
const connectedClientSchema = new Schema<IConnectedClient>({
  clientId: {
    type: String,
    required: true,
    unique: true,
    index: true,
  },
  employeeName: {
    type: String,
    required: false,
    index: true,
  },
  employeeId: {
    type: String,
    required: false,
    index: true,
  },
  employeeIdKey: {
    type: String,
    required: false,
  },
  status: {
    type: String,
    enum: ['online', 'offline'],
    default: 'online',
    index: true,
  },
  lastOfflineAlertSentAt: {
    type: Date,
    required: false,
  },
  systemInfo: {
    osName: { type: String },
    osVersion: { type: String },
    hostname: { type: String },
    cpuModel: { type: String },
    cpuCores: { type: Number },
    totalRamGb: { type: Number },
    totalDiskGb: { type: Number },
    architecture: { type: String },
  },
  resourceUsage: {
    sampledAt: { type: Date },
    cpuPercent: { type: Number, min: 0, max: 100 },
    memoryUsedBytes: { type: Number, min: 0 },
    memoryTotalBytes: { type: Number, min: 0 },
    diskUsedBytes: { type: Number, min: 0 },
    diskTotalBytes: { type: Number, min: 0 },
  },
  firstSeen: {
    type: Date,
    required: true,
    default: Date.now,
  },
  lastSeen: {
    type: Date,
    required: true,
    default: Date.now,
    index: true,
  },
}, {
  timestamps: true,
  collection: 'connected_clients',
});

/**
 * Client Config Schema
 */
const clientConfigSchema = new Schema<IClientConfig>({
  employeeName: {
    type: String,
    required: true,
    unique: true,
    index: true,
  },
  serverUrl: {
    type: String,
    required: true,
  },
  authToken: {
    type: String,
    required: true,
  },
  screenshotIntervalMinutes: {
    type: Number,
    required: true,
    default: 10,
    min: 1,
  },
  dataSendIntervalMinutes: {
    type: Number,
    required: true,
    default: 10,
    min: 1,
  },
  locationUpdateIntervalMinutes: {
    type: Number,
    required: true,
    default: 30,
    min: 1,
  },
  idleThresholdSeconds: {
    type: Number,
    required: true,
    default: 300,
    min: 1,
  },
  appUsagePollIntervalSeconds: {
    type: Number,
    required: true,
    default: 2,
    min: 0.5,
  },
  screenshotQuality: {
    type: Number,
    required: true,
    default: 75,
    min: 1,
    max: 100,
  },
  logLevel: {
    type: String,
    required: true,
    default: 'INFO',
    enum: ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'],
  },
  fileDownloadPath: {
    type: String,
    required: true,
    default: 'C:\\Downloads\\CompanyFiles',
  },
  version: {
    type: Number,
    required: true,
    default: 1,
    index: true,
  },
}, {
  timestamps: true,
  collection: 'client_configs',
});
employeeSchema.index({ managerId: 1, name: 1 });
employeeSchema.index({ managerIds: 1, name: 1 });
// Sparse unique indexes are backward compatible with legacy rows that predate
// employee-ID identity. Those rows acquire a key the next time they report.
employeeSchema.index({ externalEmployeeId: 1 }, { unique: true, sparse: true });
connectedClientSchema.index({ employeeIdKey: 1 }, { unique: true, sparse: true });

const managerAccountSchema = new Schema<IManagerAccount>({
  employeeId: { type: Schema.Types.ObjectId, ref: 'Employee', required: true, unique: true, index: true },
  email: { type: String, required: true, unique: true, lowercase: true, trim: true, index: true },
  passwordHash: { type: String, required: true },
}, { timestamps: true, collection: 'manager_accounts' });

const serverConfigSchema = new Schema<IServerConfig>({
  key: { type: String, required: true, unique: true, default: 'global' },
  port: { type: Number, required: true, min: 1, max: 65535 },
  nodeEnv: { type: String, required: true, enum: ['development', 'production', 'test'] },
  mongodbUri: { type: String, required: true },
  screenshotStoragePath: { type: String, required: true },
  screenshotTtlDays: { type: Number, required: true, min: 1 },
  clientAuthToken: { type: String, required: true },
  jwtSecret: { type: String, required: true },
  logLevel: { type: String, required: true, enum: ['error', 'warn', 'info', 'debug'] },
  shiftStartHour: { type: Number, required: true, min: 0, max: 23 },
  shiftEndHour: { type: Number, required: true, min: 0, max: 23 },
  fileUploadPath: { type: String, required: true },
  maxFileUploadSizeMb: { type: Number, required: true, min: 1 },
  eodReportTime: { type: String, required: true, match: /^([01]\d|2[0-3]):[0-5]\d$/ },
}, {
  timestamps: true,
  collection: 'server_configs',
});

const serverStatisticSchema = new Schema<IServerStatistic>({
  timestamp: { type: Date, required: true, index: true },
  cpuPercent: { type: Number, required: true, min: 0, max: 100 },
  memoryUsedBytes: { type: Number, required: true, min: 0 },
  memoryTotalBytes: { type: Number, required: true, min: 0 },
  diskUsedBytes: { type: Number, required: true, min: 0 },
  diskTotalBytes: { type: Number, required: true, min: 0 },
  expiresAt: { type: Date, required: true, expires: 0 },
}, { collection: 'server_statistics' });

/**
 * EOD Report Config Document Interface
 */
export interface IEODReportConfig extends Document {
  _id: mongoose.Types.ObjectId;
  clientId: string;
  employeeName: string;
  email: string;
  enabled: boolean;
  createdAt: Date;
  updatedAt: Date;
}

/**
 * EOD Report Config Schema
 */
const eodReportConfigSchema = new Schema<IEODReportConfig>({
  clientId: {
    type: String,
    required: true,
    // Uniqueness/index declared explicitly below via schema.index() to avoid a
    // duplicate-index definition.
  },
  employeeName: {
    type: String,
    required: true,
  },
  email: {
    type: String,
    required: true,
    match: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
  },
  enabled: {
    type: Boolean,
    required: true,
    default: true,
  },
}, {
  timestamps: true,
  collection: 'eod_report_configs',
});

// One EOD report configuration per user/client. Keyed on clientId alone (not
// clientId+email) so the same employee cannot be registered twice and receive
// duplicate daily reports.
eodReportConfigSchema.index({ clientId: 1 }, { unique: true });

/**
 * Export Models
 */
export const Employee = mongoose.model<IEmployee>('Employee', employeeSchema);
export const ManagerAccount = mongoose.model<IManagerAccount>('ManagerAccount', managerAccountSchema);
export const ActivityLog = mongoose.model<IActivityLog>('ActivityLog', activityLogSchema);
export const Screenshot = mongoose.model<IScreenshot>('Screenshot', screenshotSchema);
export const ConnectedClient = mongoose.model<IConnectedClient>('ConnectedClient', connectedClientSchema);
export const ServerStatistic = mongoose.model<IServerStatistic>('ServerStatistic', serverStatisticSchema);
export const ClientConfig = mongoose.model<IClientConfig>('ClientConfig', clientConfigSchema);
export const ServerConfigModel = mongoose.model<IServerConfig>('ServerConfig', serverConfigSchema);
export const EODReportConfig = mongoose.model<IEODReportConfig>('EODReportConfig', eodReportConfigSchema);
