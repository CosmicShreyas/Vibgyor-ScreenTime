...
router.put('/', validateJwtToken, async (req, res) => {
  try {
    ...
    if (updates.otpAdminEmail !== undefined) {
      if (typeof updates.otpAdminEmail !== 'string') {
        res.status(400).json({ error: 'OTP admin email must be a string' });
        return;
      }
      const otpAdminEmail = updates.otpAdminEmail.trim();
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!otpAdminEmail || !emailRegex.test(otpAdminEmail)) {
        res.status(400).json({ error: 'Enter a valid OTP admin email address' });
        return;
      }
      updates.otpAdminEmail = otpAdminEmail;
    }
