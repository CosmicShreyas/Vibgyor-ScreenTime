...
router.post('/heartbeat', validateClientToken, async (req, res) => {
  try {
    const { client_id, employee_name, employee_id, paused, resource_usage } = req.body || {};
    if (!client_id || typeof client_id !== 'string') {
      res.status(400).json({ error: 'client_id is required' });
      return;
    }

    await connectedClientService.registerClient(client_id, employee_name, employee_id, undefined, false, resource_usage);
    if (employee_name) {
      await employeeService.touchLastSeenByName(employee_name, !!paused);
      websocketService.notifyEmployeeUpdate(employee_name, 'status_change', {
...
router.post('/data', validateClientToken, async (req, res) => {
  try {
    ...
    await connectedClientService.registerClient(clientId, employeeName, employeeId, systemInfo, false, resourceUsage);
    ...
    try {
      ...
      const tamper = monitoringPayload.tamper;
      if (tamper && (
        tamper.process_restarted ||
        tamper.relaunched_by_watchdog ||
        tamper.watchdog_restarted ||
        tamper.clock_jump_detected ||
        tamper.autostart_restored
      )) {
        websocketService.notifyEmployeeUpdate(displayName, 'monitoring_recovery', {
          tamper,
          timestamp: monitoringPayload.timestamp,
        });
      }
