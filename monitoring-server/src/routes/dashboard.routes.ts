...
/**
 * GET /api/employees/timesheet/range
 * Returns a daily-breakdown timesheet for an inclusive date range.
 * Query params: startDate, endDate (YYYY-MM-DD, maximum 31 calendar days)
 */
router.get('/timesheet/range', validateJwtToken, async (req: DashboardRequest, res) => {
  try {
    ...
    if (!isValidDate(startDate) || !isValidDate(endDate) || startDate > endDate) {
      res.status(400).json({ error: 'Valid startDate and endDate are required (YYYY-MM-DD)' });
      return;
    }
    ...
    if (rangeDays > 31) {
      res.status(400).json({ error: 'Timesheet ranges cannot exceed 31 calendar days' });
      return;
    }

    logger.info(`Fetching timesheet range: ${startDate} to ${endDate}`);
    const timesheetData = await employeeService.getTimesheetReport(startDate, endDate, await scope(req));
    res.status(200).json(timesheetData);
  } catch (error) {
    logger.error('Error fetching timesheet range:', error);
    res.status(500).json({ error: 'Failed to fetch timesheet range' });
  }
});
