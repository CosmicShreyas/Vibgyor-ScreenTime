...
const router = Router();
const managerNames = async (req: any) => req.user?.role === 'manager' && req.user.managerEmployeeId ? managerService.getAssignedEmployeeNames(req.user.managerEmployeeId) : undefined;
const managerCanAccess = async (req: any, name: string) => req.user?.role !== 'manager' || !req.user.managerEmployeeId || managerService.canAccessEmployee(req.user.managerEmployeeId, name);
...
router.get('/public/wellbeing/focus/:name', async (req: Request, res: Response) => {
  try {
    ...
    if (!(await managerCanAccess(req, req.params.name))) return res.status(404).json({ error: 'Employee not found' });
...
router.get('/overview', validateJwtToken, async (req: Request, res: Response) => {
  try {
    ...
    if (employee && !(await managerCanAccess(req, String(employee)))) return res.status(404).json({ error: 'Employee not found' });
    const start = (startDate || date) as string | undefined;
    if (employee && !(await managerCanAccess(req, employee as string))) return res.status(404).json({ error: 'Employee not found' });
    const overview = await analyticsService.getTeamOverview(start, endDate as string | undefined, employee as string | undefined, await managerNames(req));
    res.json(overview);
  } catch (error: any) {
...
router.get('/trend', validateJwtToken, async (req: Request, res: Response) => {
  try {
    ...
    const endDate = req.query.endDate as string | undefined;
    if (employee && !(await managerCanAccess(req, employee))) return res.status(404).json({ error: 'Employee not found' });
    const trend = await analyticsService.getProductivityTrend(days, employee, endDate, await managerNames(req));
    res.json(trend);
  } catch (error: any) {
...
router.get('/insights', validateJwtToken, async (req: Request, res: Response) => {
  try {
    ...
    const start = (startDate || date) as string | undefined;
    if (employee && !(await managerCanAccess(req, employee as string))) return res.status(404).json({ error: 'Employee not found' });
    const insights = await analyticsService.getInsights(start, endDate as string | undefined, employee as string | undefined, await managerNames(req));
    res.json({ insights });
  } catch (error: any) {
...
router.get('/alerts', validateJwtToken, async (req: Request, res: Response) => {
  try {
    ...
    const scopedNames = await managerNames(req);
    const scopeFilteredAlerts = scopedNames ? allAlerts.filter((alert: any) => scopedNames.some(name => String(alert.employee_name || '').toLowerCase() === name.toLowerCase())) : allAlerts;
