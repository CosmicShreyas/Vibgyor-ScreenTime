...
import { toDateStr } from '../utils/timezone';
...
router.post('/send-now', validateJwtToken, async (req: Request, res: Response) => {
  try {
    const { clientId, employeeName, email, date } = req.body;

    if (!clientId || !employeeName || !email) {
      return res.status(400).json({ error: 'clientId, employeeName, and email are required' });
    }

    // A manual test send reports on the requested day, defaulting to TODAY so the
    // tester sees current activity. The scheduled job (send-all) uses the previous
    // completed day instead.
    const targetDate = typeof date === 'string' && date ? date : toDateStr(new Date());
    await eodReportsService.sendEODReport(clientId, employeeName, email, targetDate);
