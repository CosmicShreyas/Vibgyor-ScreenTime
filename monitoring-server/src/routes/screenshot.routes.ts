...
router.get('/:id', validateJwtToken, async (req, res) => {
  try {
    ...
    // Older records may persist a relative path (e.g. 'screenshots/...' when the
    // storage dir default './screenshots' was used). res.sendFile requires an
    // absolute path, so normalize against the server's working directory.
    const filePath = path.resolve(screenshot.file_path);

    // Check if file exists
    try {
      await fs.stat(filePath);
    } catch (error) {
      logger.error(`Screenshot file not found on disk: ${filePath}`, error);
