    ...
    async fn spawn_monitoring_tasks(&self) -> Result<()> {
        ...
        // Task 2.5: Browser tab tracker update (polls every 10 seconds like app usage)
        {
            ...
            tokio::spawn(async move {
                let poll_interval = {
                    let cfg = config.read();
                    Duration::from_secs_f64(cfg.app_usage_poll_interval_seconds.clamp(0.5, 2.0))
                };
...
async fn main() -> Result<()> {
    ...
    // Installer-only reset used before starting/replacing the recovery task.
    if args.iter().any(|arg| arg == "--clear-stop") {
    ...
    if args.iter().any(|a| stop_args.contains(&a.as_str())) {
    ...
    // Windows recovery launches must respect a deliberate Stop. A normal user
    // or installer start clears a stale marker; the once-a-minute scheduled
    // recovery must not silently undo an intentional stop.
    let scheduled_launch = args.iter().any(|arg| arg == "--scheduled");
    if scheduled_launch && resilience::stop_requested() {
        return Ok(());
    }
    ...
    // Shared transmitter for tray-status and employee-information callbacks.
    // Pause/resume is posted immediately; the periodic heartbeat retries it.
    ...
    if let Some(ref tray) = system_tray {
        ...
        let pause_transmitter = Arc::clone(&http_transmitter_for_callback);
        let pause_gui_state = Arc::clone(&gui_state);
        let pause_client_id = client_id.clone();
        let pause_fallback_name = employee_info.employee_name.clone();
        tray.set_on_pause_click(move || {
            info!("⏸️ Pause monitoring");
            *paused_flag_clone.write() = true;
            let transmitter = Arc::clone(&pause_transmitter);
            let heartbeat_url = transmitter.sibling_url("heartbeat");
            let employee_name = pause_gui_state
                .get_current_info()
                .map(|info| info.employee_name)
                .unwrap_or_else(|| pause_fallback_name.clone());
            let client_id = pause_client_id.clone();
            tokio::spawn(async move {
                let payload = serde_json::json!({
                    "client_id": client_id,
                ...
                if let Err(error) = transmitter.post_json(&heartbeat_url, &payload).await {
                    debug!(
                        "Immediate pause heartbeat failed (periodic heartbeat will retry): {}",
                        error
                    );
                }
            });
        ...
        let paused_flag_clone = Arc::clone(&paused_flag);
        let resume_transmitter = Arc::clone(&http_transmitter_for_callback);
        let resume_gui_state = Arc::clone(&gui_state);
        let resume_client_id = client_id.clone();
        let resume_fallback_name = employee_info.employee_name.clone();
        tray.set_on_resume_click(move || {
            info!("▶️ Resume monitoring");
            *paused_flag_clone.write() = false;
            let transmitter = Arc::clone(&resume_transmitter);
            let heartbeat_url = transmitter.sibling_url("heartbeat");
            let employee_name = resume_gui_state
                .get_current_info()
                .map(|info| info.employee_name)
                .unwrap_or_else(|| resume_fallback_name.clone());
            let client_id = resume_client_id.clone();
            tokio::spawn(async move {
                let payload = serde_json::json!({
                    "client_id": client_id,
                    "employee_name": employee_name,
                ...
                if let Err(error) = transmitter.post_json(&heartbeat_url, &payload).await {
                    debug!(
                        "Immediate resume heartbeat failed (periodic heartbeat will retry): {}",
                        error
                    );
                }
            });
    ...
    // Always use the distinct companion watchdog. A watchdog-launched client
    // inherits the existing watchdog PID and adopts it, avoiding a supervision
    // gap or duplicate companion during recovery. Otherwise, start a new one.
    let inherited_watchdog_pid = std::env::var(resilience::WATCHDOG_PID_ENV)
        .ok()
        .and_then(|value| value.parse::<u32>().ok())
        .filter(|pid| resilience::process_alive(*pid));
    let watchdog_pid = Arc::new(RwLock::new(
        inherited_watchdog_pid.or_else(resilience::spawn_watchdog),
    ));
    ...
    // watchdog process was itself killed, spawn a fresh one.
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if gap > 90 {
                    warn!(
                        "🛡️ Resume detected (~{}s gap) — skipping watchdog revival this cycle",
                        gap
                    );
                    continue;
                }

                // Revive the fallback watchdog only if it is confirmed gone across two
                ...
                if !alive_now {
                    ...
                    if still_gone {
                        ...
                        *watchdog_pid.write() = resilience::spawn_watchdog();
                        tamper_state.mark_watchdog_restarted();
                    }
                }
            }
        });
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match updater.check_and_apply_update(&cfg_snapshot).await {
                    ...
                    Err(e) => warn!(
                        "Periodic OTA check failed (will retry next interval): {}",
                        e
                    ),
                }
            }
