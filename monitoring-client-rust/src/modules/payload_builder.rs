...
pub struct PayloadBuilder {
    ...
    /// Resilience/anti-tamper state; its signals ride along with each payload.
    tamper_state: crate::modules::resilience::TamperState,
}
...
impl PayloadBuilder {
    ...
    /// * `system_info` - System information (optional, only sent on first run)
    ///
    /// # Returns
    ...
    /// let payload = builder.build_payload("John Doe", None, "client-123", None).await?;
    ...
    pub async fn build_payload(
        ...
        client_id: &str,
        system_info: Option<crate::modules::system_info::SystemInfo>,
    ) -> Result<Payload> {
        // Validate client_id
        ...
        // Get activity data
        let activity = self.activity_tracker.get_activity_payload();
        ...
        let has_system_info = system_info.is_some();
        let payload = Payload {
            ...
            employee_id: employee_id.map(|id| id.trim().to_string()),
            ...
            idle_reason: self.activity_tracker.take_idle_reason(),
            location,
            system_info,
            tamper: self.tamper_state.take_report(),
        };

        info!(
            "Built payload for client {}: {}s work, {}s idle, {} apps, {} tabs, screenshot: {}, system_info: {}",
            client_id,
...
mod tests {
    ...
    fn test_serialize_payload() {
        ...
        let payload = Payload {
            ...
            activity: ActivityData::basic(100, 50),
