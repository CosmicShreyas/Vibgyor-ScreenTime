...
///   VS_REQUIRED  = "1" | "0" require a non-empty value and prevent dismissal
///
...
$requiredInput = (EnvOr 'VS_REQUIRED' '0') -eq '1'
...
if ($kind -eq 'input') {
  ...
  $cancelVis = if ($requiredInput) { 'Collapsed' } else { 'Visible' }
  $buttonsXaml = @"
      <Button x:Name='BtnCancel' Content='Cancel' Style='{StaticResource Ghost}' Margin='0,0,10,0' Visibility='$cancelVis'/>
  ...
  <Border CornerRadius='16' Background='$cBg' BorderBrush='$cBorder' BorderThickness='1' Margin='12'>
    ...
    <Grid>
      ...
      <StackPanel Grid.Row='2' Margin='26,10,26,24'>
        ...
        <TextBlock x:Name='ValidationText' Text='A reason is required before this window can be closed.'
                   Foreground='#F87171' FontSize='12' FontWeight='SemiBold'
                   Margin='4,8,0,0' Visibility='Collapsed'/>
...
$validationText = $win.FindName('ValidationText')
...
function Submit-Input {
  $value = Get-Value
  if ($requiredInput -and [string]::IsNullOrWhiteSpace($value)) {
    $validationText.Visibility = 'Visible'
    $field.Focus() | Out-Null
    return
  }
  $script:result = 'NEXT' + [char]1 + $value
...
if ($kind -eq 'input') {
  ...
  $btnNext.Add_Click({ Submit-Input })
  if ($btnBack) { $btnBack.Add_Click({ $script:result = 'BACK'; $win.Close() }) }
  if ($btnCancel) { $btnCancel.Add_Click({ $script:result = 'CANCEL'; $win.Close() }) }
  if ($field) { $field.Add_TextChanged({ $validationText.Visibility = 'Collapsed' }) }
  if ($fieldPass) { $fieldPass.Add_PasswordChanged({ $validationText.Visibility = 'Collapsed' }) }
  # Mandatory prompts ignore Escape and reject all close attempts until a
  # non-empty value is submitted. This also covers Alt+F4.
  $win.Add_KeyDown({
    if ($_.Key -eq 'Return') { Submit-Input }
    elseif ($_.Key -eq 'Escape' -and -not $requiredInput) { $script:result = 'CANCEL'; $win.Close() }
  })
  $win.Add_Closing({
    if ($requiredInput -and -not $script:submitted) {
      $_.Cancel = $true
      $validationText.Visibility = 'Visible'
      if ($isPass) { $fieldPass.Focus() | Out-Null } else { $field.Focus() | Out-Null }
    }
  })
} else {
...
/// The themed tray context menu — a centered WPF window (same style/placement as
/// the setup prompt), replacing the old native Win32 tray menu. Reads:
///   VS_PAUSED   = "1" | "0"   (chooses Pause vs Resume row)
///   VS_SUBTITLE = employee line shown under the title
/// Prints the chosen action id on stdout: stats|settings|about|pause|resume
/// (empty if dismissed via Close Menu / Esc). No "stop" — stopping is handled
/// only via the uninstaller / `--stop`, not the tray menu.
#[cfg(target_os = "windows")]
...
<Window xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation'
        xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml'
        Title='ScreenTime' WindowStartupLocation='CenterScreen' SizeToContent='WidthAndHeight'
        WindowStyle='None' AllowsTransparency='True' Background='Transparent'
  ...
  <Border CornerRadius='14' Background='$cBg' BorderBrush='$cBorder' BorderThickness='1' Margin='10' Width='264'>
    ...
    <StackPanel Margin='8'>
      ...
      <Button x:Name='BtnDismiss' Style='{StaticResource Row}'>
        <StackPanel Orientation='Horizontal'>
          <TextBlock Text='&#x2715;' FontSize='14' Foreground='$cMuted' Width='24'/>
          <TextBlock Text='Close Menu' Foreground='$cMuted' VerticalAlignment='Center'/>
        </StackPanel>
      </Button>
    </StackPanel>
  </Border>
...
$win.FindName('BtnDismiss').Add_Click({ $win.Close() })
$win.Add_KeyDown({ if ($_.Key -eq 'Escape') { $win.Close() } })

# The window centers on screen (WindowStartupLocation='CenterScreen') exactly
# like the setup prompt — no cursor-anchored positioning. It stays open until
# the user picks an item, presses Esc, or clicks Close Menu (no dismiss-on-blur,
# which misbehaves for a hidden/background-launched process).
