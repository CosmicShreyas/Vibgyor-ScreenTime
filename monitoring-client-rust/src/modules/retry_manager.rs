...
impl RetryManager {
    ...
    /// # }
    /// ```
    pub fn new(queue_manager: Arc<QueueManager>, http_transmitter: Arc<HttpTransmitter>) -> Self {
        ...
        loop {
            ...
            if let Some(last_retry) = *self.last_retry_time.read() {
                ...
                if time_since_last_retry < backoff_delay {
                    ...
                    debug!(
                        "Applying backoff delay: {:.2} seconds",
                        wait_time.as_secs_f64()
                    );
                    let _ = last_retry; // Drop the guard before await
...
mod tests {
    ...
    fn test_calculate_backoff_exponential() {
        assert_eq!(RetryManager::calculate_backoff(1), Duration::from_secs(2));
        assert_eq!(RetryManager::calculate_backoff(2), Duration::from_secs(4));
        assert_eq!(RetryManager::calculate_backoff(3), Duration::from_secs(8));
        assert_eq!(RetryManager::calculate_backoff(4), Duration::from_secs(16));
    }
