...
//!   2. Watchdog relaunch: a detached companion process relaunches the main
//!      agent within seconds if it is killed; the main agent relaunches the
//!      watchdog if IT is killed (mutual revival).
...
const RUN_VALUE_NAME: &str = "VibgyorSeekMonitoring";
/// Env var the watchdog child is launched with, so it knows to run as watchdog.
pub const WATCHDOG_ENV: &str = "VIBGYOR_WATCHDOG";
...
/// Is this process the watchdog companion (launched with WATCHDOG_ENV set)?
pub fn is_watchdog() -> bool {
    std::env::var(WATCHDOG_ENV).is_ok()
}
...
pub fn spawn_watchdog() -> Option<u32> {
    ...
    match Command::new(&exe)
        .env(WATCHDOG_ENV, main_pid.to_string())
...
/// The watchdog's main loop. Runs in the child process (WATCHDOG_ENV set).
...
pub fn run_watchdog_loop() -> ! {
    ...
    let mut watched_pid: u32 = std::env::var(WATCHDOG_ENV)
        .ok()
    ...
    loop {
            ...
            .spawn()
        {
            Ok(child) => {
                ...
                // The new main agent will spawn its own watchdog; this one can retire
                // to avoid unbounded watchdog accumulation.
