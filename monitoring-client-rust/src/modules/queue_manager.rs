...
impl QueueManager {
    ...
    pub fn new(db_path: Option<impl AsRef<Path>>) -> Result<Self> {
        ...
        let connection = Connection::open(&db_path)
            .map_err(|e| MonitoringError::Queue(format!("Failed to open database: {}", e)))?;
    ...
    fn init_database(&self) -> Result<()> {
        ...
        .map_err(|e| MonitoringError::Queue(format!("Failed to create timestamp index: {}", e)))?;
    ...
    pub fn add(&self, payload: JsonValue) -> Result<()> {
        ...
        let payload_json = serde_json::to_string(&payload)
            .map_err(|e| MonitoringError::Queue(format!("Failed to serialize payload: {}", e)))?;
    ...
    pub fn retrieve(&self, limit: usize) -> Result<Vec<(i64, JsonValue)>> {
        ...
        let rows = stmt
            ...
            .map_err(|e| MonitoringError::Queue(format!("Failed to retrieve payloads: {}", e)))?;

        let mut results = Vec::new();
        for row in rows {
            match row {
                Ok((id, payload_json)) => match serde_json::from_str::<JsonValue>(&payload_json) {
    ...
    pub fn delete(&self, payload_id: i64) -> Result<bool> {
        ...
        let rows_affected = conn
            .execute(
                "DELETE FROM payload_queue WHERE id = ?1",
                params![payload_id],
            )
            .map_err(|e| {
    ...
    pub fn size(&self) -> Result<usize> {
        ...
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM payload_queue", [], |row| row.get(0))
    ...
    pub fn increment_retry_count(&self, payload_id: i64) -> Result<bool> {
        ...
        if rows_affected > 0 {
            ...
            warn!("Payload {} not found for retry count update", payload_id);
            Ok(false)
        }
    }
