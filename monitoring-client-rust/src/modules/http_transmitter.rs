...
impl HttpTransmitter {
    ...
    /// Sends a lightweight JSON POST to an arbitrary URL (e.g. the heartbeat
    /// endpoint) reusing this transmitter's client and auth token. Best-effort:
    /// returns an error on failure but callers typically just log and continue.
    pub async fn post_json(&self, url: &str, payload: &JsonValue) -> Result<()> {
        let response = self
            .client
            .post(url)
            .header("Content-Type", "application/json")
            ...
            .send()
            .await
            .map_err(MonitoringError::Network)?;
    ...
    /// Derives a sibling endpoint URL by replacing the last path segment of the
    /// configured data URL. E.g. `.../api/monitoring/data` -> `.../api/monitoring/heartbeat`.
    pub fn sibling_url(&self, last_segment: &str) -> String {
        match self.server_url.rfind('/') {
            Some(idx) => format!("{}/{}", &self.server_url[..idx], last_segment),
            None => format!("{}/{}", self.server_url, last_segment),
        }
    }
...
mod tests {
    ...
    fn test_new_transmitter() {
        let transmitter = HttpTransmitter::new("https://api.example.com/data", "test-token", None);
    ...
    fn test_new_transmitter_with_custom_timeout() {
        let transmitter =
            HttpTransmitter::new("https://api.example.com/data", "test-token", Some(60));
        assert!(transmitter.is_ok());
    ...
    fn test_new_transmitter_empty_url() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            MonitoringError::Transmission(_)
        ));
    }
    ...
    fn test_new_transmitter_trims_whitespace() {
        let transmitter =
            HttpTransmitter::new("  https://api.example.com/data  ", "  test-token  ", None);
        assert!(transmitter.is_ok());
    ...
    async fn test_send_payload_null() {
        let transmitter =
            HttpTransmitter::new("https://api.example.com/data", "test-token", None).unwrap();
