...
impl ScreenshotCapture {
    ...
    fn capture_single_screen(
        ...
        let rgb_image = ImageBuffer::from_raw(width, height, rgb_data).ok_or_else(|| {
    ...
    fn capture_multiple_screens(
        ...
        // Guard against invalid geometry (monitor hot-plug/unplug, negative offsets,
        // or overflow) which would otherwise panic the allocation below and, since
        // this runs inside a spawned task, silently kill screenshot capture entirely.
        if max_x <= min_x || max_y <= min_y {
            return Err(MonitoringError::Screenshot(format!(
                "Invalid combined screen bounds: x[{}..{}] y[{}..{}]",
        ...
        // Cap total pixel area to avoid pathological allocations (e.g. bogus bounds).
        const MAX_PIXELS: u64 = 100_000_000; // ~100 MP; covers large multi-4K setups
        let area = total_width as u64 * total_height as u64;
        if total_width == 0 || total_height == 0 || area > MAX_PIXELS {
            return Err(MonitoringError::Screenshot(format!(
                "Unreasonable combined screenshot size: {}x{}",
...
mod tests {
    ...
    fn test_capture_screenshot_no_panic() {
        ...
        match capture.capture_screenshot() {
            ...
            Err(e) => {
                println!(
                    "Screenshot capture failed (expected in headless environment): {}",
                    e
                );
            }
        }
