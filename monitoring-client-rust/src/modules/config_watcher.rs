...
impl ConfigWatcher {
    ...
    async fn fetch_and_apply_config(&self) -> Result<bool> {
        ...
        if !response.status().is_success() {
            warn!(
                "⚠️ Failed to fetch config: HTTP {} - keeping existing .env file",
                response.status()
            );
            return Ok(false);
        }
    ...
    pub fn write_env_file(&self, config_data: &serde_json::Value) -> Result<()> {
        ...
        let server_url = config_data["SERVER_URL"]
            .as_str()
            .unwrap_or(&current_config.server_url);
        let auth_token = config_data["AUTH_TOKEN"]
            .as_str()
            .unwrap_or(&current_config.auth_token);
        let screenshot_interval = config_data["SCREENSHOT_INTERVAL_MINUTES"]
            .as_u64()
            .unwrap_or(current_config.screenshot_interval_minutes as u64);
        let data_send_interval = config_data["DATA_SEND_INTERVAL_MINUTES"]
            .as_u64()
            .unwrap_or(current_config.data_send_interval_minutes as u64);
        let location_interval = config_data["LOCATION_UPDATE_INTERVAL_MINUTES"]
            .as_u64()
            .unwrap_or(current_config.location_update_interval_minutes as u64);
        let idle_threshold = config_data["IDLE_THRESHOLD_SECONDS"]
            .as_u64()
            .unwrap_or(current_config.idle_threshold_seconds as u64);
        let app_poll_interval = config_data["APP_USAGE_POLL_INTERVAL_SECONDS"]
            .as_f64()
            .unwrap_or(current_config.app_usage_poll_interval_seconds);
        let screenshot_quality = config_data["SCREENSHOT_QUALITY"]
            .as_u64()
            .unwrap_or(current_config.screenshot_quality as u64);
        let log_level = config_data["LOG_LEVEL"].as_str().unwrap_or("INFO");

        // Anti-cheat / heartbeat / idle-reason (fall back to current config).
        let genuine_enabled = config_data["GENUINE_DETECTION_ENABLED"]
            .as_bool()
            .unwrap_or(current_config.genuine_detection_enabled);
        let genuine_window = config_data["GENUINE_WINDOW_SECONDS"]
            .as_u64()
            .unwrap_or(current_config.genuine_window_seconds as u64);
        let genuine_threshold = config_data["GENUINE_FLAG_THRESHOLD"]
            .as_u64()
            .unwrap_or(current_config.genuine_flag_threshold as u64);
        let heartbeat_interval = config_data["HEARTBEAT_INTERVAL_SECONDS"]
            .as_u64()
            .unwrap_or(current_config.heartbeat_interval_seconds as u64);
        let idle_reason_minutes = config_data["IDLE_REASON_PROMPT_MINUTES"]
            .as_u64()
            .unwrap_or(current_config.idle_reason_prompt_minutes as u64);

        // Client-only settings the server does NOT normally manage. These MUST be
        // preserved across a server-driven rewrite, otherwise OTA auto-update and
        // the self-view link would be silently wiped on every config poll. Prefer
        // a server-sent value if present, else carry the current local value.
        let dashboard_url = config_data["DASHBOARD_URL"]
            .as_str()
            .unwrap_or(&current_config.dashboard_url);
        let ota_enabled = config_data["OTA_ENABLED"]
            .as_bool()
            .unwrap_or(current_config.ota_enabled);
        let ota_repo = config_data["OTA_GITHUB_REPO_URL"]
            .as_str()
            .unwrap_or(&current_config.ota_github_repo_url);
        let ota_on_startup = config_data["OTA_CHECK_ON_STARTUP"]
            .as_bool()
            .unwrap_or(current_config.ota_check_on_startup);
        let ota_interval = config_data["OTA_CHECK_INTERVAL_MINUTES"]
            .as_u64()
            .unwrap_or(current_config.ota_check_interval_minutes as u64);
        let ota_asset = config_data["OTA_RELEASE_ASSET_NAME"]
            .as_str()
            .map(|s| s.to_string())
            .or_else(|| current_config.ota_release_asset_name.clone())
            .unwrap_or_default();
        let ota_exe = config_data["OTA_EXECUTABLE_NAME"]
            .as_str()
            .map(|s| s.to_string())
            .or_else(|| current_config.ota_executable_name.clone())
            .unwrap_or_default();
        // Only emit the DASHBOARD_URL line when we actually have a value, so an
        // empty value stays commented-out/absent rather than writing a blank key.
        let dashboard_line = if dashboard_url.trim().is_empty() {
            String::new()
        } else {
...
# Genuine-activity detection (anti-cheat)
GENUINE_DETECTION_ENABLED={}
GENUINE_WINDOW_SECONDS={}
GENUINE_FLAG_THRESHOLD={}

# Heartbeat & idle-reason prompt
HEARTBEAT_INTERVAL_SECONDS={}
IDLE_REASON_PROMPT_MINUTES={}

# Dashboard (web UI) base URL for the tray "View Your Stats" self-view link.
{}
# GitHub OTA (over-the-air) auto-update configuration.
OTA_ENABLED={}
OTA_GITHUB_REPO_URL={}
OTA_CHECK_ON_STARTUP={}
