...
mod tests {
    ...
    fn test_employee_info_manager() {
        ...
        let manager = EmployeeInfoManager::new(Some(info_path.clone()), "admin123".to_string());

        // Initially no info
        assert!(!manager.info_exists());

        // Create/update info (password verification is a separate step, see below).
        let info = manager
            .update_info("John Doe".to_string(), "EMP001".to_string())
            .unwrap();
        ...
        // Password verification is handled by verify_password(), not update_info().
        assert!(manager.verify_password("admin123"));
        assert!(!manager.verify_password("wrongpassword"));
    }
