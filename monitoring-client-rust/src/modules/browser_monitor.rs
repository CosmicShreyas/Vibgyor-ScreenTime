...
/// Title used for browser time that can't be attributed to a specific tab (new
/// tab page, settings, PDF viewer, devtools, transient title-parse miss). Ensures
/// the sum of tab durations tracks the browser's foreground app-time instead of
/// silently under-counting.
const FALLBACK_TAB_TITLE: &str = "(Browsing — untitled tab)";
...
impl BrowserMonitor {
    ...
    pub fn get_active_browser_tab(&self) -> Result<Option<BrowserTab>, MonitoringError> {
        #[cfg(target_os = "windows")]
        {
            ...
            unsafe {
                ...
                let process_browser = self.browser_for_pid(pid);
                let active_url = extract_active_url(hwnd.0);
                ...
                if class_name == "Chrome_WidgetWin_1" {
                    let (title_browser, title_private) = classify_chromium_window(&title);
                    let browser = process_browser.or(title_browser);
                    if let Some(browser) = browser {
                        let is_private = title_private
                            || (browser == Browser::Chrome && title.contains("Incognito"))
                            || (browser == Browser::Edge && title.contains("InPrivate"));
                        let stripped = strip_browser_suffix(&title, browser);
                        // When the window has a real tab title, use it; otherwise
                        // (new-tab page, settings, empty title, transient parse
                        // miss) fall back to a placeholder for THIS browser so the
                        // time the browser is focused is still credited to a tab.
                        // Without this, browser app-time > sum(tab-time) because
                        // these moments leaked from tab tracking. See fallback below.
                        let tab_title = clean_tab_title(&stripped)
                            .unwrap_or_else(|| FALLBACK_TAB_TITLE.to_string());
                ...
                } else if let Some(browser) = process_browser {
                    // Foreground window belongs to a browser process but its class
                    // isn't a standard tabbed-window class (e.g. a PDF viewer host,
                    // devtools, a task-manager/picture-in-picture window). Still
                    // credit the browser so its app-time and tab-time stay in sync.
                    return Ok(Some(BrowserTab {
                        browser: browser_label(browser, false),
                        title: FALLBACK_TAB_TITLE.to_string(),
                        url: active_url,
                    }));
                }
    ...
    /// Resolve a foreground window's browser from its owning process. This is
    /// reliable for Chrome/Edge windows whose title omits the browser suffix.
    #[cfg(target_os = "windows")]
    fn browser_for_pid(&self, pid: u32) -> Option<Browser> {
        if pid == 0 {
            return None;
        }
        ...
        let name = system.process(pid)?.name().to_ascii_lowercase();
        if name == "chrome.exe" || name == "chrome" {
            Some(Browser::Chrome)
        } else if name == "msedge.exe" || name == "msedge" {
            Some(Browser::Edge)
        } else if name == "firefox.exe" || name == "firefox" {
...
/// Group + deduplicate exact tab entries. URL-aware active entries remain
/// distinct so the dashboard can group a repeated title and reveal its URLs.
fn group_tabs(tabs: Vec<BrowserTab>) -> Vec<BrowserTab> {
    let mut seen: HashSet<String> = HashSet::new();
    let mut grouped: Vec<BrowserTab> = Vec::new();
    for tab in tabs {
        let key = format!("{}|{}|{}", tab.browser, tab.title, tab.url);
        if seen.insert(key) {
            grouped.push(tab);
        }
    }
...
/// Read the active browser window's address field through UI Automation.
/// Only address-like Edit controls are considered; browser-internal URLs and
/// arbitrary search-box text are deliberately discarded.
#[cfg(target_os = "windows")]
fn extract_active_url(hwnd: isize) -> String {
    ...
    let window = match automation.element_from_handle(Handle::from(hwnd)) {
        Ok(value) => value,
        Err(_) => return String::new(),
    };
    ...
    let edits = match window.find_all(TreeScope::Subtree, &condition) {
        Ok(value) => value,
        Err(_) => return String::new(),
    };
    for edit in edits {
        let name = edit.get_name().unwrap_or_default().to_ascii_lowercase();
        let automation_id = edit
            .get_automation_id()
            .unwrap_or_default()
            .to_ascii_lowercase();
        let address_like = name.contains("address")
        ...
        let value = edit
            ...
            .unwrap_or_default();
        if let Some(url) = normalize_url_candidate(&value, address_like) {
            return url;
        }
    }
    String::new()
...
fn normalize_url_candidate(raw: &str, address_like: bool) -> Option<String> {
    let value = raw.trim();
    ...
    let explicit_url = lower.starts_with("http://") || lower.starts_with("https://");
    let domain_like = address_like
        && value.contains('.')
        && !value.chars().any(char::is_whitespace)
        && value.len() <= 2048;
    if explicit_url || domain_like {
...
impl BrowserTabUsageTracker {
    ...
    pub fn new(browser_monitor: Arc<BrowserMonitor>) -> Self {
        ...
        Self {
            ...
            last_full_scan: Arc::new(RwLock::new(Instant::now() - Duration::from_secs(30))),
        }
    }
    ...
    pub fn update(&self) -> Result<(), MonitoringError> {
        ...
        // Full UIA enumeration is materially heavier than foreground capture.
        // Scan the complete tab tree periodically, while sampling the active
        // foreground window on every short poll.
        let should_full_scan = self.current_tabs.read().is_empty()
            || self.last_full_scan.read().elapsed() >= Duration::from_secs(30);
        let open_tabs = if should_full_scan {
            let tabs = self.browser_monitor.get_browser_tabs()?;
            *self.last_full_scan.write() = current_time;
            tabs
        } else {
            Vec::new()
        };
        ...
        let mut merged = self.current_tabs.read().clone();
        for tab in open_tabs {
            let key = Self::get_tab_key(&tab);
            if !merged.iter().any(|item| Self::get_tab_key(item) == key) {
                merged.push(tab);
            }
        }
    ...
    pub fn reset_interval(&self) {
        ...
        self.current_tabs.write().clear();
        *self.last_update.write() = Instant::now();
        *self.last_full_scan.write() = Instant::now() - Duration::from_secs(30);
...
mod tests {
    ...
    fn test_tab_key_generation() {
        ...
        assert_eq!(key, "Chrome|GitHub|https://github.com");
    ...
    #[test]
    fn test_group_tabs_preserves_distinct_urls() {
        let tabs = vec![
            BrowserTab {
                browser: "Chrome".into(),
                title: "GitHub".into(),
        ...
        assert!(grouped.iter().any(|tab| tab.url == "https://github.com/x"));
    ...
    #[test]
    fn test_normalize_url_candidate() {
        assert_eq!(
            normalize_url_candidate("https://example.com/a", false),
            Some("https://example.com/a".into())
        );
        assert_eq!(
            normalize_url_candidate("example.com/a", true),
            Some("example.com/a".into())
        );
        assert_eq!(normalize_url_candidate("quarterly report", true), None);
        assert_eq!(normalize_url_candidate("chrome://settings", true), None);
    }
}
