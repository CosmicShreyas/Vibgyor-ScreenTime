...
impl OTPClient {
    ...
    pub async fn request_otp(&self, client_id: &str, employee_name: &str, employee_id: &str) -> Result<String> {
        ...
        if !response.status().is_success() {
            ...
            return Err(MonitoringError::Config(format!("OTP request failed: {}", error_text)));
        }
    ...
    pub async fn verify_otp(&self, client_id: &str, otp: &str) -> Result<()> {
        ...
        if !response.status().is_success() {
            ...
            return Err(MonitoringError::Config(format!("OTP verification failed: {}", error_text)));
        }
