...
pub struct SystemTray {
    ...
    /// Paused state mirrored as an atomic the tray thread polls, so it can swap
    /// the tray icon to the red-dot "paused" variant and back.
    #[cfg(target_os = "windows")]
    paused_icon_flag: Arc<AtomicBool>,
}
...
impl SystemTray {
    ...
    pub fn new() -> Result<Self> {
        ...
        let paused_icon_flag = Arc::new(AtomicBool::new(false));
        let paused_icon_flag_thread = Arc::clone(&paused_icon_flag);

        // Create tray icon in a separate thread
        std::thread::spawn(move || {
            // Build both icon variants once (normal + paused-with-red-dot) and
            // leak them for 'static lifetime the trayicon crate requires.
            let normal_bytes: &'static [u8] =
                Box::leak(Self::create_icon_data(false).into_boxed_slice());
            let paused_bytes: &'static [u8] =
                Box::leak(Self::create_icon_data(true).into_boxed_slice());
            let normal_icon = Icon::from_buffer(normal_bytes, Some(32), Some(32))
                .expect("Failed to create tray icon");
            let paused_icon = Icon::from_buffer(paused_bytes, Some(32), Some(32))
            ...
            // Pump Windows messages AND poll the paused flag so we can swap the
            // icon (red dot when paused). We use PeekMessage + a short sleep
            // instead of a blocking GetMessage so the flag is checked regularly.
            #[cfg(target_os = "windows")]
            unsafe {
                ...
                loop {
                    ...
                    // Reflect the paused state in the icon when it changes.
                    let want_paused = paused_icon_flag_thread.load(Ordering::Relaxed);
                    if want_paused != shown_paused {
                        let icon = if want_paused {
                            &paused_icon
                        } else {
                            &normal_icon
                        };
    ...
    fn dispatch_pause(&self) {
        ...
        *self.is_paused.write() = true;
        self.paused_icon_flag.store(true, Ordering::Relaxed);
    ...
    fn dispatch_resume(&self) {
        ...
        *self.is_paused.write() = false;
        self.paused_icon_flag.store(false, Ordering::Relaxed);
    ...
    ///
    /// When `paused` is true, a red status dot (white-ringed for contrast) is
    /// painted in the bottom-right corner so the tray shows monitoring is paused.
    #[cfg(target_os = "windows")]
    fn create_icon_data(paused: bool) -> Vec<u8> {
        const DIM: u32 = 32;

        // Decode the embedded PNG → RGBA8, resized to DIM×DIM.
        let mut rgba: Vec<u8> = match image::load_from_memory(Self::BRAND_ICON_PNG) {
            Ok(img) => {
        ...
        if paused {
            Self::paint_paused_dot(&mut rgba, DIM);
        }

        Self::rgba_to_ico(&rgba, DIM)
    }

    /// Paint a red "paused" status dot with a white ring into the bottom-right of
    /// a top-down RGBA8 `dim`×`dim` buffer.
    #[cfg(target_os = "windows")]
    fn paint_paused_dot(rgba: &mut [u8], dim: u32) {
        let d = dim as i32;
        // Dot centered near the bottom-right; radius scales with icon size.
        let r = (dim as f32 * 0.30).round() as i32; // outer (white ring) radius
        let inner = (r as f32 * 0.68).round() as i32; // red fill radius
        let cx = d - r - 1;
        let cy = d - r - 1;
        let put = |buf: &mut [u8], x: i32, y: i32, rr: u8, gg: u8, bb: u8| {
            if x < 0 || y < 0 || x >= d || y >= d {
                return;
            }
            let idx = ((y * d + x) * 4) as usize;
            buf[idx] = rr;
            buf[idx + 1] = gg;
            buf[idx + 2] = bb;
            buf[idx + 3] = 0xFF;
        ...
        for y in (cy - r)..=(cy + r) {
            for x in (cx - r)..=(cx + r) {
                ...
                if dist2 <= inner * inner {
                    put(rgba, x, y, 0xEF, 0x44, 0x44); // red fill (#EF4444)
                } else if dist2 <= r * r {
                    put(rgba, x, y, 0xFF, 0xFF, 0xFF); // white ring
                }
            }
        }
    }
