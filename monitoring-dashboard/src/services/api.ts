...
const api = axios.create({
  baseURL: '/api',
  // Guard against a slow/huge response hanging the UI indefinitely. 45s is
  // generous for the heaviest analytics range while still failing fast enough
  // to show an error instead of an eternal spinner.
  timeout: 45000,
  headers: {
...
export interface Alert {
  ...
  threshold: number
  created_at: string
  /** Legacy response text returned by older servers. */
  idle_reasons?: string[]
  idle_responses?: IdleResponse[]
}
...
export const employeeService = {
  ...
  getTimesheetRange: async (startDate: string, endDate: string): Promise<any[]> => {
    const response = await api.get('/employees/timesheet/range', {
...
export const selfViewService = {
  getDetail: async (name: string, startDate?: string, endDate?: string): Promise<EmployeeDetail> => {
    const response = await publicApi.get(`/employees/public/${encodeURIComponent(name)}`, { params: { startDate, endDate } })
    return response.data
  },
  getApplicationUsage: async (name: string, startDate?: string, endDate?: string): Promise<ApplicationUsage> => {
    const response = await publicApi.get(`/employees/public/${encodeURIComponent(name)}/app-usage`, { params: { period: 'today', startDate, endDate } })
    return response.data
  },
  getBrowserTabUsage: async (name: string, startDate?: string, endDate?: string): Promise<BrowserTabUsage> => {
    const response = await publicApi.get(`/employees/public/${encodeURIComponent(name)}/browser-tab-usage`, { params: { period: 'today', startDate, endDate } })
    return response.data
  },
  getWeeklyTimeline: async (name: string): Promise<any> => {
    const response = await publicApi.get(`/employees/public/${encodeURIComponent(name)}/weekly-timeline`)
    return response.data
  },
...
export const screenshotService = {
  ...
  /**
   * Paginated screenshot list. Returns a page of `limit` items plus the `total`
   * count for the range, so the gallery can page server-side instead of pulling
   * every screenshot at once.
   */
  getScreenshotsWithFilters: async (
    ...
    // Backward-safe: server now returns {items,total}; tolerate an old array too.
  ...
  /**
   * Lightweight {id, employee_name, captured_at} index for the timeline
   * tooltip's nearest-capture thumbnail lookup. Scope to one employee to keep
   * the payload tiny (the hover only needs the selected employee's captures).
   */
  getScreenshotIndex: async (
    startDate: string,
    endDate: string,
    employeeName?: string
  ): Promise<Array<{ id: string; employee_name: string; captured_at: string }>> => {
    const params = new URLSearchParams({ startDate, endDate })
    if (employeeName) params.append('employeeName', employeeName)
    const response = await api.get(`/screenshots/index?${params.toString()}`)
    return response.data
  },

  getScreenshotSummaries: async (
    startDate: string,
    endDate: string
  ): Promise<Array<{ employee_name: string; latest_screenshot: { id: string; captured_at: string }; total_count: number }>> => {
    const params = new URLSearchParams({ startDate, endDate })
    const response = await api.get(`/screenshots/summary?${params.toString()}`)
    return response.data
  },
}
