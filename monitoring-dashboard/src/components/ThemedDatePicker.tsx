...
const formatDisplayDate = (value: string, selectionMode: 'day' | 'week' | 'month') => {
  const date = toDate(value)
  if (!date) return 'Select date'
  if (selectionMode === 'month') return `${monthNames[date.getMonth()]} ${date.getFullYear()}`
  if (selectionMode === 'week') {
    ...
    const short = (day: Date) => day.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
...
const isSameWeek = (first: Date | null, second: Date | null) => {
  if (!first || !second) return false
  ...
  return monday(first) === monday(second)
}

export default function ThemedDatePicker({ value, onChange, selectionMode = 'day', min, max, className = '' }: ThemedDatePickerProps) {
  ...
  return (
    <>
      ...
      >
        <span>{formatDisplayDate(value, selectionMode)}</span>
          ...
          >
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] font-semibold text-[var(--muted-foreground)]">
                  Select a {selectionMode === 'day' ? 'date' : selectionMode}
                </p>
              </div>
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {calendarDays.map((date) => {
                ...
                const selectedDateValue = toDate(value)
                const selected = selectionMode === 'month'
                  ? !!selectedDateValue && date.getFullYear() === selectedDateValue.getFullYear() && date.getMonth() === selectedDateValue.getMonth()
                  : selectionMode === 'week'
                    ? isSameWeek(date, selectedDateValue)
