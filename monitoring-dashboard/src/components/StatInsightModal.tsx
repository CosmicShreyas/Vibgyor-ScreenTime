      ...
      >
        ...
        >
          ...
          <div className="overflow-y-auto p-5 sm:p-6">
            {details && (details.captures || details.calculation || details.interpretation || details.caveat) && (
              <div className="mb-4 grid gap-3 md:grid-cols-2">
                {details.captures && <ExplanationBox label="What it captures" text={details.captures} color={color} />}
                {details.calculation && <ExplanationBox label="How it is calculated" text={details.calculation} color={color} />}
                {details.interpretation && <ExplanationBox label="How to read it" text={details.interpretation} color={color} />}
                {details.caveat && <ExplanationBox label="Important limitation" text={details.caveat} color="var(--warning)" />}
              </div>
            )}
...
function ExplanationBox({ label, text, color }: { label: string; text: string; color: string }) {
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[color-mix(in_oklab,var(--secondary)_44%,transparent)] p-4">
      <p className="ms-eyebrow" style={{ color }}>{label}</p>
