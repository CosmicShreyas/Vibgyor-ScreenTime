          ...
          <div className="overflow-y-auto p-5 sm:p-6">
            {(calculation || interpretation) && (
              <div className="mb-4 grid gap-3 md:grid-cols-2">
                {calculation && <MethodBox label="How it is calculated" text={calculation} color={toneColor} />}
                {interpretation && <MethodBox label="How to read it" text={interpretation} color="var(--signal)" />}
              </div>
            )}
...
function MethodBox({ label, text, color }: { label: string; text: string; color: string }) {
  return (
