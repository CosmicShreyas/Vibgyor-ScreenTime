...
interface EmployeeTimelineProps {
  ...
  /**
   * Optional: called the first time the user hovers the timeline, so the parent
   * can lazily fetch the screenshot index (kept off the initial load path).
   */
  onHoverStart?: () => void
}

export default function EmployeeTimeline({ timelines, shiftStartHour, shiftEndHour, screenshotLookup, onHoverStart }: EmployeeTimelineProps) {
  ...
  // rAF-throttle the hover: raw mousemove fires dozens of times per second, but
  // recomputing the tooltip (screenshot lookup + React state update) more than
  // once per frame is wasted work that made hovering feel laggy/stuck. We keep
  // only the latest pointer position and process it once per animation frame.
  const rafId = useRef<number | null>(null)
  const pending = useRef<{ timeline: TimelineData; clientX: number; rect: DOMRect } | null>(null)

  const hoverStarted = useRef(false)

  // Stable handlers (useCallback) so the memoized rows below never rebuild when
  // only the tooltip state changes. Without this, every rAF tooltip update
  // re-rendered all 40 timeline rows — the source of the hover lag.
  const handleTimelineHover = useCallback((timeline: TimelineData, event: React.MouseEvent<HTMLDivElement>) => {
    // Kick off the lazy screenshot-index load on the very first hover only.
    if (!hoverStarted.current) {
      hoverStarted.current = true
      onHoverStart?.()
    }
    // Read pooled-event values synchronously, then defer the heavy work.
    pending.current = {
      timeline,
    ...
    rafId.current = requestAnimationFrame(() => {
      ...
      const next = computeTimelineTooltip(
        p.timeline.name,
        p.timeline.segments,
        p.timeline.work_time_today,
        p.timeline.idle_time_today,
        { clientX: p.clientX, rect: p.rect },
        shiftStartHour,
        shiftHours,
        screenshotLookup
      )
  ...
  }, [shiftStartHour, shiftHours, screenshotLookup, onHoverStart])
