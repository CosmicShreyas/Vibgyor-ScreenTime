...
const ServerConfigTab: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await api.put('/server-config', payload);
      showMessage('success', response.data.message || 'Server configuration saved to the database and applied.');
    } catch (error: any) {
      showMessage('error', error.response?.data?.error || 'Failed to save server configuration');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Database-backed runtime settings. Changes persist across restarts and live-safe values apply immediately.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Port number for the API server (applies on the next server restart)
              </p>
            </div>
            ...
            <div>
              ...
              <ThemedSelect
                ...
                onChange={(nextValue) => handleChange('nodeEnv', nextValue)}
                options={[
            ...
            <div>
              ...
              <ThemedSelect
                ...
                onChange={(nextValue) => handleChange('logLevel', nextValue)}
                options={[
        ...
        <div className="bg-gray-50 dark:bg-gray-700/50 p-6 rounded-lg">
          ...
          <div>
            ...
            <input
              ...
              className="w-full cursor-not-allowed px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg opacity-70
                       bg-white dark:bg-gray-700 text-gray-900 dark:text-white
                       focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Bootstrap-only connection string. It remains read-only because the server must connect before database settings can be loaded.
            </p>
          </div>
        </div>
