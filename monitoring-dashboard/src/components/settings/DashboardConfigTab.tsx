...
const DashboardConfigTab: React.FC = () => {
  ...
  const fetchConfig = async () => {
    ...
    try {
      ...
      setConfig(prev => ({
        ...
        otpAdminEmail: response.data.otpAdminEmail || '',
      }));
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(config.otpAdminEmail.trim())) {
      showMessage('error', 'Enter a valid admin email for employee update OTP verification');
      setSaving(false);
      return;
    }
    ...
    try {
      const updates: any = {
        ...
        otpAdminEmail: config.otpAdminEmail.trim(),
      };
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Manage dashboard credentials, OTP recipients, and notification preferences
        </p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gray-50 dark:bg-gray-700/50 p-6 rounded-lg">
          ...
          <div className="space-y-4">
            ...
            {/* Employee update OTP recipient */}
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Admin Email for OTP Verification
              </label>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                <input
                  type="email"
                  value={config.otpAdminEmail}
                  onChange={(e) => handleChange('otpAdminEmail', e.target.value)}
              ...
              <p className="mt-1.5 text-xs text-gray-600 dark:text-gray-400">
                Receives OTP codes when a client requests permission to change employee information. This replaces the ADMIN_EMAIL environment setting.
              </p>
            </div>
