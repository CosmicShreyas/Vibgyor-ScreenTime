 ...
 * @param pointer       Cursor X + the row's bounding rect (captured from the
 *                      mouse event by the caller, so this stays synchronous-safe
 *                      and can be deferred to an animation frame).
...
export function computeTimelineTooltip(
  ...
  workSeconds: number,
  idleSeconds: number,
  pointer: { clientX: number; rect: DOMRect },
  ...
  if (screenshotLookup && anchor) {
    const frac = Math.min(1, Math.max(0, (pointer.clientX - rect.left) / Math.max(1, rect.width)))
  ...
  return {
    ...
    thumbnailUrl,
    hoverTime,
    x: Math.round(pointer.clientX / 20) * 20,
    y: rect.top - 10,
  }
}
...
export default function TimelineTooltip({ tooltip }: { tooltip: TimelineTooltipData | null }) {
  // Defer the screenshot thumbnail until the cursor RESTS on a spot. Screenshots
  // are full-resolution JPEGs (often hundreds of KB); fetching a new one on every
  // hover bucket while sweeping the timeline caused a storm of large image
  // downloads — the actual lag. We only commit the thumbnail URL after it has
  // stayed the same for REST_MS, so dragging across the bar fetches nothing and
  // pausing to look fetches exactly one image (then browser-cached on return).
  const REST_MS = 400
  const [restingThumb, setRestingThumb] = useState<string | null>(null)
  const wantedThumb = tooltip?.thumbnailUrl ?? null

  useEffect(() => {
    if (!wantedThumb) {
      setRestingThumb(null)
      return
    }
    if (wantedThumb === restingThumb) return
    const t = window.setTimeout(() => setRestingThumb(wantedThumb), REST_MS)
    return () => window.clearTimeout(t)
  }, [wantedThumb, restingThumb])

  if (!tooltip) return null
  // Only show the image once it matches the rested URL (avoids showing a stale
  // thumbnail from a previous hover position while a new one settles).
  const showThumb = restingThumb && restingThumb === wantedThumb ? restingThumb : null
  return (
    // Portal to <body> so the fixed-positioned card is placed relative to the
    // VIEWPORT. Without this, the tooltip renders inside the timeline card, which
    // sits under an animated framer-motion `motion.div` whose transform becomes
    // the containing block for `position: fixed` — sending the tooltip far from
    // the cursor ("somewhere down there") and thrashing layout.
    <Portal>
    <div
        ...
        <div className="space-y-2">
          ...
          {showThumb && (
            <div className="mb-2 overflow-hidden rounded-lg border border-white/15">
              <img
                src={showThumb}
                alt="Screenshot near hovered time"
                loading="lazy"
                decoding="async"
