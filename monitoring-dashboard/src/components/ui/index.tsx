...
import { enrichStatInsightDetails } from '../metricExplanations'
...
export function StatTile({
  ...
  const effectiveDetails = enrichStatInsightDetails(label, details)
  ...
  const content = (
    <>
      ...
      <span className="mt-4 flex items-center gap-1 text-[11px] font-semibold uppercase tracking-wider" style={{ color: toneColor }}>
        {onClick ? 'View details' : 'How this metric works'} <ChevronRight size={13} className="transition-transform group-hover:translate-x-0.5" />
      </span>
    </>
  )
