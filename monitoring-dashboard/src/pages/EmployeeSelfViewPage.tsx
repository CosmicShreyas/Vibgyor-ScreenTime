...
import { enrichStatInsightDetails } from '../components/metricExplanations'
...
export default function EmployeeSelfViewPage() {
  ...
  const loadEmployeeData = async () => {
    try {
      ...
      const [detail, appData, tabData, timelineData, focus] = await Promise.all([
        selfViewService.getDetail(identifier, startDate, endDate),
        selfViewService.getApplicationUsage(identifier, startDate, endDate),
        selfViewService.getBrowserTabUsage(identifier, startDate, endDate),
        selfViewService.getWeeklyTimeline(identifier),
        selfViewService.getFocusMetrics(identifier, rangeDays, endDate).catch(() => null),
      ])
      ...
      if (detail) {
        ...
        setTimeline([
          {
            name: detail.name,
            status: detail.monitoring_paused ? 'paused' : 'active',
  ...
  const tabRows = useMemo(
    () => (tabUsage?.browser_tabs || []).filter((t) => t.duration > 0),
    [tabUsage]
  )
  ...
  const focusDays = wellbeing?.days ?? []
    ...
    <div className="ms-aurora-bg min-h-screen px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1200px] space-y-5">
        ...
        <motion.header
          ...
          style={{
            background:
              'radial-gradient(120% 140% at 0% 0%, color-mix(in oklab, var(--primary) 16%, transparent), transparent 55%), radial-gradient(120% 140% at 100% 0%, color-mix(in oklab, var(--signal) 14%, transparent), transparent 60%), color-mix(in oklab, var(--card) 52%, transparent)',
        ...
        {employee.monitoring_paused && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="flex gap-3 rounded-2xl border border-[color-mix(in_oklab,var(--danger)_55%,var(--border))] bg-[color-mix(in_oklab,var(--danger)_13%,var(--card)_62%)] p-4 backdrop-blur-xl"
            role="status"
          >
            <PauseCircle className="mt-0.5 shrink-0 text-[var(--danger)]" size={22} />
            <div>
              <p className="font-display font-semibold text-[var(--foreground)]">You have paused your monitoring</p>
              <p className="mt-1 text-sm text-[var(--muted-foreground)]">
                ScreenTime remains connected and reports only that monitoring is paused. Activity, applications, browser tabs, location, and screenshots are not collected until you resume.
              </p>
            </div>
          </motion.div>
        )}
          ...
          {wellbeing ? (
            <>
              <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                <MiniStat
                  label="Avg focus / day"
                  value={formatMinutes(wellbeing.avg_focus_minutes ?? 0)}
                  icon={Brain}
                  tone="var(--primary)"
                  modalTone="primary"
                  hint={`${activeFocusDays.length} active day${activeFocusDays.length === 1 ? '' : 's'} in this range`}
                  details={{ eyebrow: 'Deep-work time', description: 'Average time per active day spent in uninterrupted work blocks of at least 15 minutes. A gap longer than 3 minutes ends a block. Total work time also includes short and fragmented active periods, so it is normally higher.', itemLabel: 'days', items: dailyFocusItems }}
                />
                <MiniStat
                  label="Avg switches / day"
                  value={`${wellbeing.avg_context_switches ?? 0}`}
                  icon={Activity}
                  tone="var(--signal)"
                  modalTone="signal"
                  hint="Foreground application changes"
                  details={{ eyebrow: 'Context switching', description: 'Average number of times the dominant foreground application changed per active day.', itemLabel: 'days', items: focusDays.map((day: any) => ({ label: day.date, value: `${day.context_switches} switches`, numeric: day.context_switches, secondary: `${day.focus_minutes}m focus · ${day.work_minutes}m total work` })) }}
                />
                <MiniStat
                  label="Best focus day"
                  value={formatMinutes(wellbeing.best_focus_minutes ?? 0)}
                  sub={wellbeing.best_focus_day || undefined}
                  icon={Flame}
                  tone="var(--success)"
                  modalTone="success"
                  hint="Highest focus total in the range"
                  details={{ eyebrow: 'Best deep-work day', description: 'The selected-range day with the highest combined duration of qualifying uninterrupted focus blocks.', itemLabel: 'days', items: dailyFocusItems }}
                />
                <MiniStat
                  label="Flow score"
                  value={`${wellbeing.flow_score ?? 0}`}
                  icon={TrendingUp}
                  tone={scoreTone(wellbeing.flow_score ?? 0) === 'success' ? 'var(--success)' : scoreTone(wellbeing.flow_score ?? 0) === 'warning' ? 'var(--warning)' : 'var(--danger)'}
                  modalTone={scoreTone(wellbeing.flow_score ?? 0)}
                  hint="Latest day in the selected range"
                  details={{ eyebrow: 'Focus quality', description: 'A 0–100 daily signal that rewards total focus time and the longest uninterrupted block, while penalizing frequent application switching.', itemLabel: 'days', items: focusDays.map((day: any) => ({ label: day.date, value: `${day.focus_minutes}m focus`, numeric: day.focus_minutes, secondary: `${day.context_switches} switches · ${day.longest_focus_minutes}m longest block` })) }}
                />
              </div>
              {focusChart.length > 0 ? (
        ...
        <MotionCard accent className="p-5">
          ...
          />
          {tabRows.length > 0 ? (
            <BrowserUsageBreakdown tabs={tabRows} />
          ) : (
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-1.5">
            {Icon && <Icon size={13} className="shrink-0" style={{ color: tone || 'var(--muted-foreground)' }} />}
      ...
      <StatInsightModal
        ...
        hint={hint ?? sub}
        tone={modalTone}
        details={enrichStatInsightDetails(label, details)}
      />
    </>
  )
}
...
function UsageBars({
  ...
  return (
    ...
    <div className="custom-scrollbar max-h-[280px] space-y-3 overflow-y-auto p-3">
      {rows.map((row, idx) => {
