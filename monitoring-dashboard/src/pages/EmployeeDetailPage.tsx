...
import api, { employeeService, EmployeeDetail } from '../services/api'
...
export default function EmployeeDetailPage() {
  ...
  const loadEmployeeDetail = async () => {
    try {
      const [data, timelineData, systemResponse] = await Promise.all([
        ...
        api.get(`/connected-clients/by-employee/${encodeURIComponent(name!)}`).catch(() => null),
      ])
      setEmployee(data)
      ...
      if (systemResponse) {
        ...
        setResourceUsage(systemData.resourceUsage)
      }
    } catch (error) {
  ...
  return (
    ...
    >
        ...
        <MotionCard className="p-5">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
            <StatTile
              ...
              details={{
                eyebrow: 'Application classification',
                description: `Productivity classification for ${rangeLabel}.`,
                captures: 'Foreground application duration is matched against configurable keyword rules. Examples include Excel, Word, Teams, Visual Studio, Jira, Figma, and GitHub as productive; browsers, Explorer, search, and WhatsApp as neutral; and entertainment, social, streaming, games, and Steam as unproductive. Unknown applications default to neutral.',
                calculation: 'Score = (productive seconds + 0.5 × neutral seconds) ÷ (productive + neutral + unproductive seconds) × 100, rounded to a whole percent. When no categorized application time exists, the score is 0%.',
                interpretation: 'Productive time receives full credit, neutral time half credit, and unproductive time no credit. The score describes the application mix, not work quality, output, or employee intent.',
                caveat: 'The score and three category cards use foreground application durations. Website domains are classified and shown separately below, but their tab durations are not added to this score.',
                items: [
                  { label: 'Productive', value: formatDuration(employee.productivity.category_seconds.productive), numeric: employee.productivity.category_seconds.productive, secondary: '100% weight in score' },
                  { label: 'Neutral', value: formatDuration(employee.productivity.category_seconds.neutral), numeric: employee.productivity.category_seconds.neutral, secondary: '50% weight in score' },
                  { label: 'Unproductive', value: formatDuration(employee.productivity.category_seconds.unproductive), numeric: employee.productivity.category_seconds.unproductive, secondary: '0% weight in score' },
                ],
            ...
            <StatTile
              ...
              tone="success"
              details={{
                eyebrow: 'Application classification', description: `Productive application time for ${rangeLabel}.`,
                captures: 'Foreground duration from applications whose name matches a productive keyword, such as Excel, Word, Outlook, Teams, AutoCAD, Photoshop, Visual Studio, ERP, Slack, Zoom, Jira, Notion, Figma, Gmail, GitHub, or GitLab. Organization overrides can change these rules.',
                calculation: 'Sum the non-negative durations of all foreground application records classified as productive across the selected date range.',
                interpretation: 'This bucket contributes 100% of its duration to the productivity-score numerator.',
                caveat: 'A productive label means the application matched a rule; it does not inspect the content created or judge whether the time produced useful output.',
              }}
            />
            <StatTile
              label="Neutral"
              ...
              tone="signal"
              details={{
                eyebrow: 'Application classification', description: `Neutral application time for ${rangeLabel}.`,
                captures: 'Foreground duration from applications matched as neutral—such as Chrome, Firefox, Edge, Explorer, search, or WhatsApp—plus every unknown application that matches no configured rule.',
                calculation: 'Sum the non-negative durations of all foreground application records classified as neutral across the selected date range.',
                interpretation: 'Neutral time is neither rewarded fully nor penalized: it contributes 50% of its duration to the productivity-score numerator.',
                caveat: 'Unknown applications intentionally default to neutral to avoid an automatic penalty. An administrator can override classifications through the server’s application-category configuration.',
              }}
            />
            <StatTile
              label="Unproductive"
              ...
              tone="danger"
              details={{
                eyebrow: 'Application classification', description: `Unproductive application time for ${rangeLabel}.`,
                captures: 'Foreground duration from applications whose name matches an unproductive keyword, including common streaming, social-media, entertainment, gaming, and Steam rules. Organization overrides can reclassify any keyword.',
                calculation: 'Sum the non-negative durations of all foreground application records classified as unproductive across the selected date range.',
                interpretation: 'This bucket remains in the score denominator but contributes nothing to its numerator, so a larger share lowers the score.',
                caveat: 'This is a configurable rule match, not a judgment of context. A normally recreational service may still be work-related for some roles.',
              }}
            />
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            <StatTile
              ...
              details={{
                eyebrow: 'Privacy-safe input intensity', description: `Keyboard activity rate for ${rangeLabel}.`,
                captures: 'The client counts keyboard input events only. It never records which keys were pressed or the text entered.',
                calculation: `Total counted keystrokes (${employee.integrity.keystrokes}) ÷ total tracked minutes (work seconds + idle seconds), rounded to one decimal. The denominator has a one-minute minimum to keep short or empty ranges stable.`,
                interpretation: 'This is an activity-intensity measure, not a typing-speed or productivity score. Reading, meetings, design, and other legitimate work can naturally have a low rate.',
                caveat: 'Counts depend on the client input hook being available. Hook failure disables detailed scoring and the client falls back safely to ordinary idle detection.',
              }}
            />
            <StatTile
              ...
              details={{
                eyebrow: 'Privacy-safe input intensity', description: `Mouse action rate for ${rangeLabel}.`,
                captures: `Mouse clicks and scroll events are counted. The displayed total beneath the card is clicks only (${employee.integrity.mouse_clicks}); mouse movement distance is collected for integrity analysis but is not included in this rate.`,
                calculation: '(total mouse clicks + total scroll events) ÷ total tracked minutes (work seconds + idle seconds), rounded to one decimal. The denominator has a one-minute minimum.',
                interpretation: 'This measures interaction frequency, not useful output. Low mouse activity is normal for typing or reading; high activity is not automatically productive.',
                caveat: 'The system analyzes counts, timing, and movement geometry—not clicked content or screen content.',
              }}
            />
            <StatTile
              label="Genuineness"
              ...
              details={{
                eyebrow: 'Anti-automation confidence', description: `Average input-pattern genuineness for ${rangeLabel}.`,
                captures: 'Privacy-safe 60-second input windows are checked for mouse-only or keyboard-only stretches, unnaturally regular timing, repeated mouse vectors, repeated click positions, and tiny no-click/no-key jiggle patterns. At least 8 input events are required to judge a window.',
                calculation: 'For each judged window, weighted suspicion signals are added and capped at 100; genuineness = 100 − suspicion. The client averages judged windows per uploaded interval, and this card is the unweighted average of the interval scores in the selected range.',
                interpretation: '100% means the judged input showed none of the enabled suspicious patterns; lower values mean more or stronger automation-like signals. The default fake-activity threshold is suspicion ≥ 70 (genuineness ≤ 30), and it is configurable.',
                caveat: 'If there are no scored intervals, the server displays 100% as the safe default. Treat this as “no suspicious evidence received,” not proof that activity was human.',
              }}
            />
            <StatTile
              ...
              tone="warning"
              details={{
                eyebrow: 'Flagged input duration', description: `Time associated with input windows that crossed the configured suspicion threshold during ${rangeLabel}.`,
                captures: 'Intervals flagged by corroborating anti-automation signals such as robotic timing, identical repeated clicks, repeated movement vectors, or mouse-jiggler-like micro-movements.',
                calculation: 'The client evaluates the recent scoring window about twice per second. Each flagged poll adds approximately 0.5 seconds; uploaded suspected-fake seconds are summed across the selected range.',
                interpretation: 'Flagged seconds are removed from work time and added to idle time before totals are uploaded, preventing suspected automation from increasing recorded work.',
                caveat: 'This is deterministic risk detection, not conclusive proof of cheating. Review the displayed suspicion reasons and surrounding activity before taking action.',
              }}
            />
          </div>
        ...
        <MotionCard className="p-5">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            <StatTile
              ...
              details={{
                eyebrow: 'Endpoint inventory', description: 'Operating-system information reported by the employee client.',
                captures: 'The operating-system family/name and version exposed by the endpoint.',
                calculation: 'The latest connected-client system-information value is displayed directly; no scoring or date-range aggregation is applied.',
                interpretation: 'Use this for compatibility and support context.', caveat: 'Unknown means the current client did not report a usable value.',
              }}
            />
            <StatTile
              label="Hostname"
              ...
              details={{
                eyebrow: 'Endpoint inventory', description: 'Identity and architecture of the monitored endpoint.',
                captures: 'The computer hostname and reported processor architecture.',
                calculation: 'The latest connected-client values are displayed directly; no scoring or date-range aggregation is applied.',
                interpretation: 'Use these values to identify which endpoint is reporting for this employee.', caveat: 'A hostname is device metadata and can be changed by an administrator.',
              }}
            />
            <StatTile
              label="Processor"
              ...
              details={{
                eyebrow: 'Endpoint inventory', description: 'Processor information reported by the monitored endpoint.',
                captures: 'CPU model text and the core count supplied by the client.',
                calculation: 'The latest client values are displayed directly. Long model names are shortened on the card.',
                interpretation: 'Use this as hardware context when diagnosing client performance.', caveat: 'Core terminology and model detail depend on what the operating system reports.',
              }}
            />
            <StatTile
              ...
              details={{
                eyebrow: 'Endpoint inventory', description: 'Installed memory and total storage capacity reported by the endpoint.',
                captures: 'Total RAM and total disk capacity—not current free space or utilization.',
                calculation: 'RAM is displayed to one decimal place and disk capacity to the nearest whole gigabyte from the latest client report.',
                interpretation: 'These are device-capacity facts, not activity or productivity measurements.', caveat: 'Reported capacity can differ from marketed capacity because unit conventions vary.',
              }}
            />
          </div>
        </MotionCard>
      ...
      {resourceUsage && (
        <MotionCard className="p-5">
          <SectionHeader eyebrow="Live endpoint telemetry" title="Current Resource Usage" icon={Activity} />
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <StatTile label="CPU Usage" value={`${Number(resourceUsage.cpuPercent || 0).toFixed(1)}%`} icon={Cpu} tone="primary" hint="Sampled every 60 seconds" />
            <StatTile label="RAM Usage" value={`${resourceUsage.memoryTotalBytes ? ((resourceUsage.memoryUsedBytes / resourceUsage.memoryTotalBytes) * 100).toFixed(1) : '0.0'}%`} icon={Server} tone="signal" hint={`${(Number(resourceUsage.memoryUsedBytes || 0) / 1024 ** 3).toFixed(1)} / ${(Number(resourceUsage.memoryTotalBytes || 0) / 1024 ** 3).toFixed(1)} GB`} />
            <StatTile label="Disk Usage" value={`${resourceUsage.diskTotalBytes ? ((resourceUsage.diskUsedBytes / resourceUsage.diskTotalBytes) * 100).toFixed(1) : '0.0'}%`} icon={HardDrive} tone="warning" hint={`${(Number(resourceUsage.diskUsedBytes || 0) / 1024 ** 3).toFixed(1)} / ${(Number(resourceUsage.diskTotalBytes || 0) / 1024 ** 3).toFixed(1)} GB`} />
          </div>
        </MotionCard>
      )}
              ...
              return (
                <div key={dayIndex} className="border-b border-[var(--border)] pb-4 last:border-b-0">
                  ...
                  <div
                    ...
                    onMouseMove={(e) => {
                      const pointer = { clientX: e.clientX, rect: e.currentTarget.getBoundingClientRect() }
