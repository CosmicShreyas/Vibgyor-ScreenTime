...
import { analyticsService, employeeService, TeamOverview, ProductivityTrendPoint, Alert, EmployeeSummary } from '../services/api'
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    employeeService.getAll()
      .then(setEmployees)
      .catch(() => toast.error('Failed to load employee options'))
  }, [])
  ...
  const load = async () => {
    ...
    try {
      ...
      const [ov, tr, ins, al] = await Promise.all([
        ...
        analyticsService.getOverview(startDate, endDate, selectedEmployee || undefined),
        analyticsService.getTrend(days, selectedEmployee || undefined, endDate),
        analyticsService.getInsights(startDate, endDate, selectedEmployee || undefined),
        analyticsService.getAlerts(startDate, endDate, selectedEmployee || undefined),
      ])
      setOverview(ov)
      setTrend(tr)
  ...
  const selectedSummary = selectedEmployee ? overview?.employees?.[0] ?? null : null
  ...
  const exportCsv = () => {
    ...
    downloadCsv(`Analytics_${selectedEmployee || 'Global'}_${startDate}_to_${endDate}`, rows)
  ...
  return (
    <PageShell
      ...
      title="Analytics"
      description="See how your team's productive time changes, who is performing consistently, and what happened each day. Use these insights to recognize strong work and find areas that may need support."
      icon={BarChart3}
      actions={
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <StatTile
              label={selectedEmployee ? 'Productivity' : 'Avg productivity'}
              numeric={overview?.average_productivity ?? 0}
              suffix="%"
              icon={Gauge}
              tone="primary"
              hint={selectedEmployee || 'Global workforce'}
              details={{
                eyebrow: selectedEmployee ? 'Employee productivity' : 'Team productivity',
                description: `${selectedEmployee || 'Employee'} productivity across ${startDate} to ${endDate}.`,
                itemLabel: 'employees',
                items: (overview?.employees ?? []).map((employee) => ({
                  label: employee.employee_name,
                  value: `${employee.productivity_score}%`,
                  numeric: employee.productivity_score,
                  secondary: `${formatDuration(employee.work_seconds)} work · ${formatDuration(employee.idle_seconds)} idle`,
                })),
              }}
            />
            <StatTile
              ...
              details={{
                ...
                description: 'Active alerts contributing to this total.',
                itemLabel: 'alerts',
            ...
            <StatTile
              ...
              details={{
                eyebrow: 'Generated insights',
                description: 'Automatically generated observations for the selected period.',
                itemLabel: 'insights',
                items: insights.map((insight, index) => ({ label: `Insight ${index + 1}`, value: 'Generated', secondary: insight })),
              }}
            />
          </div>
          ...
          {insights.length > 0 && (
            <MotionCard accent className="p-5">
              <SectionHeader eyebrow="Signal" title={selectedEmployee ? `${selectedEmployee}'s Insights` : "Today's Insights"} icon={Sparkles} />
              <motion.ul
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <MotionCard className="relative overflow-hidden p-5">
              ...
              <div className="mb-3 flex items-center gap-2 ms-eyebrow">
                <Trophy size={14} style={{ color: 'var(--warning)' }} /> {selectedEmployee ? 'Selected Employee' : 'Top Performer'}
              </div>
            ...
            <MotionCard className="relative overflow-hidden p-5">
              ...
              />
              <div className="mb-3 flex items-center gap-2 ms-eyebrow">
                <AlertTriangle size={14} style={{ color: selectedEmployee ? 'var(--primary)' : 'var(--danger)' }} /> {selectedEmployee ? 'Work & Idle' : 'Needs Attention'}
              </div>
              {selectedEmployee && selectedSummary ? (
                <>
                  <p className="font-display text-xl font-bold text-[var(--foreground)]">{selectedSummary.employee_name}</p>
                  <p className="ms-num mt-1 text-2xl font-bold text-[var(--primary)]">{formatDuration(selectedSummary.work_seconds)} work</p>
                  <p className="mt-1 text-sm text-[var(--muted-foreground)]">{formatDuration(selectedSummary.idle_seconds)} idle in the selected range</p>
                </>
