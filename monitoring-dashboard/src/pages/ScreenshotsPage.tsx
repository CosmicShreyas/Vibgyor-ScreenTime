...
export default function ScreenshotsPage() {
  ...
  const loadScreenshots = async () => {
    try {
      ...
      // Fetch the lightweight index (id/name/captured_at only). The grid renders
      // images lazily via /screenshots/:id, so it never needs the full docs.
      const data = await screenshotService.getScreenshotIndex(
        startDate,
        endDate,
        employeeName
      )
