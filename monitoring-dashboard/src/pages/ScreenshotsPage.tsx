...
import { employeeService, EmployeeSummary, screenshotService } from '../services/api'
...
// The gallery + summaries only need {id, employee_name, captured_at}, so we use
// the lightweight screenshot INDEX endpoint (tiny fields) rather than pulling
// full screenshot documents for the whole range — the source of the old freeze.
type ScreenshotLite = { id: string; employee_name: string; captured_at: string }

interface EmployeeScreenshotSummary {
  employeeName: string
  latestScreenshot: ScreenshotLite
  totalCount: number
}
...
export default function ScreenshotsPage() {
  ...
  const [screenshots, setScreenshots] = useState<ScreenshotLite[]>([])
  ...
  const [screenshotPageSize, setScreenshotPageSize] = useState(12)
  ...
  useEffect(() => {
    loadScreenshots()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedEmployee, startDate, endDate, screenshotPage, screenshotPageSize])
  ...
  const loadScreenshots = async () => {
    ...
    try {
      ...
      if (selectedEmployee === 'all') {
        const data = await screenshotService.getScreenshotSummaries(startDate, endDate)
        if (requestId !== screenshotRequest.current) return
        const summaries = data.map((summary) => ({
          employeeName: summary.employee_name,
          latestScreenshot: {
            id: summary.latest_screenshot.id,
            employee_name: summary.employee_name,
            captured_at: summary.latest_screenshot.captured_at,
          },
          totalCount: summary.total_count,
        }))
        setEmployeeSummaries(summaries)
        setScreenshotTotal(summaries.reduce((total, summary) => total + summary.totalCount, 0))
        setScreenshots([])
        return
      }

      const result = await screenshotService.getScreenshotsWithFilters(
        startDate,
        endDate,
        selectedEmployee,
        screenshotPageSize,
      ...
      if (requestId !== screenshotRequest.current) return
      setScreenshots(result.items.map((screenshot) => ({
        id: screenshot.id,
        employee_name: screenshot.employee_name ?? '',
        captured_at: screenshot.captured_at,
      })))
      setScreenshotTotal(result.total)
      setEmployeeSummaries([])

    } catch (error) {
      if (requestId === screenshotRequest.current) toast.error('Failed to load screenshots')
    } finally {
      if (requestId === screenshotRequest.current) setLoading(false)
    }
  }
    ...
    <PageShell
      ...
      actions={
        <button
          ...
          disabled={isCreatingBackup || screenshotTotal === 0}
        ...
        <MotionCard className="p-5">
          ...
          {loading ? (
            ...
            <Pagination
              ...
              totalItems={screenshotTotal}
              onPageChange={setScreenshotPage}
              onPageSizeChange={(size) => {
                setScreenshotPageSize(size)
                setScreenshotPage(1)
              }}
